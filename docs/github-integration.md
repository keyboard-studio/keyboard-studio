...
The Google OAuth PKCE flow ships across these files: [src/lib/googleOAuth.ts](../packages/studio/src/lib/googleOAuth.ts) (authorization + code exchange), [src/hooks/useGoogleAuth.ts](../packages/studio/src/hooks/useGoogleAuth.ts) (React hook), [src/lib/pkce.ts](../packages/studio/src/lib/pkce.ts) (shared PKCE helpers, used by both GitHub and Google), [src/lib/identity.ts](../packages/studio/src/lib/identity.ts) (`IdentitySession` discriminated union: `provider:"github" | "google"`), [src/lib/handleOAuthCallback.ts](../packages/studio/src/lib/handleOAuthCallback.ts) (`/oauth/google/callback` handling), and [src/components/SignUpPanel.tsx](../packages/studio/src/components/SignUpPanel.tsx) (renamed from `GitHubSignUpPanel.tsx`; now surfaces both identity buttons). The backend endpoint `POST /oauth/google/exchange` is served two ways from one shared core in [utilities/oauth-backend/src/google-handlers.ts](../utilities/oauth-backend/src/google-handlers.ts) (schemas: [google-schemas.ts](../utilities/oauth-backend/src/google-schemas.ts)): the standalone Fastify [server.ts](../utilities/oauth-backend/src/server.ts) (gated on `GOOGLE_OAUTH_ENABLED`) and the deployed Vercel serverless function [api/oauth/google/exchange.ts](../api/oauth/google/exchange.ts) (reachable at `/oauth/google/exchange` via the `vercel.json` rewrite). The serverless function is opt-in on the presence of `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` — absent creds return `503 google_oauth_not_configured` rather than `500`, so a GitHub-only deployment needs no Google config. See [DEPLOY.md](../utilities/oauth-backend/DEPLOY.md) §3 for the env vars.
...
2. **Option B server-side PR delivery.** The same App's **installation** (the App installed on the org account that owns the standing fork of `keymanapp/keyboards`) is used server-side to mint a short-lived **installation access token** via `@octokit/auth-app`. The installation token drives the entire git pipeline in `submitManagedPR` — fork check, tree creation, commit, branch, PR — without any user token. Required App permissions: Repository → Contents (read/write), Pull requests (read/write), Metadata (read).
   ...
   > **Installation-scope constraint (confirmed 2026-07-06).** An installation token can only act on repos where the App is **installed**. Unlike a human user, it has **no fork→upstream contributor affordance**: opening a PR whose base repo lacks the installation fails `403 "Resource not accessible by integration"` (even though the same token can *read* that repo). Consequently the managed PR's base is the studio's own **staging repo** `keyboard-studio/keyboards` — a same-repo PR (`UPSTREAM_OWNER === orgLogin` in `github-pipeline.ts`). Opening PRs directly against `keymanapp/keyboards` would require keymanapp to install the App (pull-requests scope suffices) plus a two-token pipeline change; until then, promotion staging → keymanapp is a separate downstream step.
