...
| The full Track 2 external-file import lane (file-picker `.kmn` → parse → mutate → emit round-trip) is unproven e2e — there is no spec for it (the file-picker UI does not exist yet; the lane is tracked in [#410](https://github.com/keyboard-studio/keyboard-studio/issues/410)). FR-004 is satisfied at the reducer level (T005), but the SPA import UI lane has no passing walk. | major | spec 034 FR-004; [#410](https://github.com/keyboard-studio/keyboard-studio/issues/410); [reducer.test.ts](../packages/studio/src/steps/reducer.test.ts) T005 |
