...
- `saveWorkingCopyDraft(state)` — serialize the store's *data* fields:
  ...
  - `baseIr` (`KeyboardIR | null`) and `baseKeyboard` (`BaseKeyboard | null`) — the base IR + base keyboard descriptor. **Persisted directly** (decided). The live working `ir` is **not** persisted — see the excluded group; it is re-seeded from `baseIr` on resume.
  ...
  - `desktopLocked` (`boolean`), `touchLayoutJson` (`string | null`), `touchDraft` (already-serializable: `{ charTouchEntries, suggestionResolvedChars }` | null), `galleryIntrosSeen` (`{ mechanism, touch }`).
  - **Excluded — derived/recomputed/re-seeded on resume, NOT persisted:**
    - `ir` (`KeyboardIR | null`) — **re-seeded on resume from `baseIr`** (`ir = baseIr`, the same seeding `instantiateFromBase` does at instantiation), **not** persisted and **not** re-derived by re-parsing `baseVfs` (decided). Rationale: the live `ir` is **not output-authoritative** — both the OSK preview (`useWorkingCopyTransform`) and the shipped artifact (`serializeWorkingCopy.ts:projectWorkingCopyForOutput`) project from `baseIr` + the overlay layers (`deletedNodeIds`/`deletedItemIds`, `phaseResults`, `identity`, `touchLayoutJson`) — **never** from `ir`. Carve edits live in the overlay sets, and the mutate-seam writes (`setWorkingIR`) are driven by the persisted `phaseResults`/touch data, so the edited state is recoverable without persisting `ir`. Re-parsing `baseVfs` instead would risk parser/recognizer **version-skew** shifting node IDs and `ownedByPattern`, which would break both the restored deleted-id keys and the Option-B `removalCapabilities` recompute precondition (which needs the post-`recognizePatterns` IR). **Invariant:** `ir` is intentionally not persisted — re-seeded from `baseIr` and replayed from the overlay/phase layers; revisit if `ir` ever carries state not reconstructible from those.
    - `undoStack` (`UndoEntry[]`) — **reset to `[]` on resume** (decided); carve undo history is **not** rehydrated across sessions. Rationale: `undoStack` is just an ordered trail of deleted ids (`UndoEntry = {k:'n'|'i', id}`) with no IR snapshot; the deletion **state** is already fully captured by the persisted `deletedNodeIds`/`deletedItemIds`, so edits survive and every prior deletion stays reversible via the Carve tile restore affordances + `restoreAll`. Cross-session sequential undo isn't an expected feature (browsers/editors drop the undo stack on reload while keeping the document), and a stale entry referencing a positional item id (`<nodeId>#<index>`) is a needless correctness risk. This is a **reversible decision** — the stack is trivially serializable if product later wants cross-session undo.
...
- `loadWorkingCopyDraft()` — parse + deserialize into a store-shaped object (Sets rebuilt, VFS via `createVirtualFS`, `ir` re-seeded from `baseIr`, `undoStack` reset to `[]`, `session` re-derived, `removalCapabilities` recomputed by re-running `classifyRemovalCapabilities` over the restored IR per above); returns `null` if absent/corrupt.
