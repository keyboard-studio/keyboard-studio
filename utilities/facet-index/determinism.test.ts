...
describe("facet-index determinism (FR-006, SC-004)", () => {
  it("two full builds over the same corpus are byte-identical on disk", () => {
    ...
    buildIndex({ corpusRoot: FIXTURE_CORPUS_ROOT, outPath: outA });
    buildIndex({ corpusRoot: FIXTURE_CORPUS_ROOT, outPath: outB });
  ...
  it("the in-memory index round-trips through stableStringify unchanged", () => {
    const index = buildIndex({ corpusRoot: FIXTURE_CORPUS_ROOT, outPath: "" });
  ...
  it("carries no timestamp field inside the hashed payload", () => {
    const text = stableStringify(buildIndex({ corpusRoot: FIXTURE_CORPUS_ROOT, outPath: "" }));
