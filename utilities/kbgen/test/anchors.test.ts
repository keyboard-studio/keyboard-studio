...
import { analyzeChar, scoreAnchors, parseName, availability } from '../analyze.ts';
...
// --- name parser ---
describe('parseName', () => {
  it('"B WITH HOOK" -> base b, mod HOOK', () => {
  ...
  it('"ENG" -> no base letter', () => {
    expect(parseName('LATIN SMALL LETTER ENG').base).toBeNull();
  });
});
