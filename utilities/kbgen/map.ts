...
import type { PlanResult } from "./place.js";

const mods = (shift: boolean, ralt: boolean): string[] =>
  [...(shift ? ["SHIFT"] : []), ...(ralt ? ["RALT"] : [])];
...
export interface PlacementMap {
  ...
  source: { locale?: string | null; unicodeVersion?: string | null; cldrVersion?: string | null };
  freeKeys: string[];
  summary: {
    specials: number;
  ...
  touch: TouchEntry[];
  completeness: { complete: boolean; missingBase: string[]; missingSpecial: string[] };
...
export function build(planResult: PlanResult, layout: Layout, ctx: BuildContext): PlacementMap {
  ...
  for (const p of planResult.placements) {
    ...
    const anchor: AnchorInfo = {
      via: p.via, weight: p.weight,
      baseChar: pos !== undefined ? (p.upper ? pos.upper : pos.lower) : null,
    };
    ...
    if (p.mechanism === "direct") {
      ...
      touch.push({
        ...
        position: pos !== undefined ? { row: pos.row, col: pos.col } : null,
  ...
  for (const k of layout.keys) {
    ...
    if (kp.defOut !== null) physical.push({
      char: k.lower,
      codepoint: "U+" + (k.lower.codePointAt(0) ?? 0).toString(16).toUpperCase().padStart(4, "0"),
      key: k.key, shift: false, method: "restore", modifiers: mods(false, true), output: k.lower,
      restoreOf: kp.defOut,
    });
    if (kp.shiftOut !== null) physical.push({
      char: k.upper,
      codepoint: "U+" + (k.upper.codePointAt(0) ?? 0).toString(16).toUpperCase().padStart(4, "0"),
...
export function format(map: PlacementMap): string {
  ...
  if (map.source.locale !== undefined || map.source.unicodeVersion !== undefined) {
  ...
  for (const e of map.physical) {
    ...
    const why = e.method === "restore"
      ? `restores, displaced by ${e.restoreOf ?? ""}`
      : `${e.anchor?.via ?? ""} (${e.anchor?.weight ?? ""})`;
