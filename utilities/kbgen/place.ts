...
import { analyzeChar, scoreAnchors } from "./analyze.js";
...
//   opts    : { freeSwap:false }
export function plan(chars: string[], layout: Layout, free: Set<string>, opts: { freeSwap?: boolean } = {}): PlanResult {
  ...
  const keyPlan = (id: string): KeyPlan => {
    if (!keys.has(id)) {
      ...
      if (!k) throw new Error(`Key ${id} not found in layout`);
      keys.set(id, {
        key: id, baseLower: k.lower, baseUpper: k.upper,
        defOut: null, shiftOut: null,
        raltDef: null, raltShift: null,
        skDef: [], skShift: [],
        displaced: false,
      });
    }
    return keys.get(id) as KeyPlan;
  };
  ...
  for (const item of ranked) {
    ...
    if (!isFree && opts.freeSwap) {
      ...
      const alt = [...free].find((id) => !taken.has(`${id}|direct|${String(upper)}`));
      if (alt) { anchorKey = alt; }
    }

    const directSlot = `${anchorKey}|direct|${String(upper)}`;
    const raltSlot = `${anchorKey}|ralt|${String(upper)}`;
...
export function checkComplete(planResult: PlanResult, layout: Layout, requestedChars: string[]): {
  complete: boolean;
  missingBase: string[];
  ...
  for (const k of layout.keys) {
    ...
    reachable.add(kp !== undefined && kp.defOut !== null ? kp.defOut : k.lower);
    reachable.add(kp !== undefined && kp.shiftOut !== null ? kp.shiftOut : k.upper);
    if (kp !== undefined) {
