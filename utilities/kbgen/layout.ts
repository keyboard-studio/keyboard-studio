...
function buildIndex(layout: { id: string; name: string; rows: [string, string, string][][] }): Layout {
  const keys: KeyEntry[] = [];          // [{ key, lower, upper, row, col }]
  const byChar = new Map<string, KeyEntry>();  // 'b' -> key entry, 'B' -> key entry
  const byKey = new Map<string, KeyEntry>();   // 'K_B' -> key entry
