...
import type { Layout, KeyEntry } from "./layout.js";
import { keyForChar } from "./layout.js";
import * as ucd from "./sources/ucd.js";
import * as confusables from "./sources/confusables.js";
...
function loadSupplement(): Record<string, SuppChar> {
  const suppFile = path.join(pkgRoot(), "data", "supplement.json");
  try {
    return (JSON.parse(fs.readFileSync(suppFile, "utf8")) as { chars: Record<string, SuppChar> }).chars;
  } catch {
    process.stderr.write("[WARN] supplement.json not found at " + suppFile + " -- continuing with empty supplement map\n");
    return {};
  }
}

const SUPP = loadSupplement();
...
const cp = (ch: string): string => (ch.codePointAt(0) ?? 0).toString(16).toUpperCase().padStart(4, "0");
...
export function parseName(name: string | null): NameParse {
  ...
  if (!m || m[1] === undefined) return { base: null, upper, mods: [] };
  ...
  const base = tokens[0] !== undefined && /^[A-Z]$/.test(tokens[0]) ? tokens[0] : null;
...
export function analyzeChar(ch: string): CharFeature {
  ...
  if (nfd.length > 1) {
    const chars = [...nfd];
    if (chars[0] !== undefined && !isCombining(chars[0]) && chars.slice(1).every(isCombining)) {
      decompBase = chars[0];
  ...
  return {
    ...
    nameBase: named.base,
    mods: named.mods,
    confusable: confusables.skeletonBase(ch),
    visual: supp.visual ?? null,
    phonetic: supp.ipa ?? null,
  };
}
...
export function scoreAnchors(feature: CharFeature, layout: Layout): AnchorCandidate[] {
  ...
  const add = (sourceChar: string | null, via: string): void => {
    if (!sourceChar) return;
    const k: KeyEntry | null = keyForChar(layout, sourceChar);
    if (k) out.push({ key: k.key, char: sourceChar, via, weight: WEIGHT[via] ?? 0 });
  };
...
export function availability(layout: Layout, usedLetters: string[] | Set<string>): Set<string> {
