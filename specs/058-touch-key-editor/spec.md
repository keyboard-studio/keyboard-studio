...
- **[NEEDS CLARIFICATION: contract sign-off]** Adding `TouchKeyIR.layer?: string` is a locked-type change under [spec.md](../../spec.md) §18 and needs sign-off recorded in [docs/spec-signoff.md](../../docs/spec-signoff.md). Recommended: **take it** (see FR-030 and the fallback in [contracts/touch-key-rule-join.md](contracts/touch-key-rule-join.md)). The subkey `default` field (longpress preselect) is a **second, real** serialized field the writer special-cases and [parseTouchLayout.ts](../../packages/contracts/src/parseTouchLayout.ts) currently drops, exactly like `layer` was dropped before this feature — recommended: **take both `TouchKeyIR.layer` and the subkey `default` field in the same locked-type change**.
...
6. **Given** the assigned character is a **multi-codepoint grapheme cluster** (e.g. an Indic conjunct treated as one inventory "letter"), **When** the proposal renders, **Then** it is handled **mechanically identically** to any other multi-codepoint output under the minting policy — a `T_*` id plus a synthesized rule (FR-025), not a special case.
...
4. **Given** any finding, **When** it renders, **Then** it is conveyed by icon and text (never colour alone), carries a codepoint-derived accessible name for any glyph, and is announced through a single **grid-owned** `aria-live` region with **no new debounce timer** (Decision D3). (The codebase already has several regions — TouchGallery ×2, DiagnosticsPanel; the requirement is that the grid adds exactly one for its own announcements, not that the app consolidates to one.)
...
- A `U_` id with a rule keyed on it: the rule overrides the self-output (a legitimate idiom), and MUST be reported as an **override**, not redundancy — unless the rule's output equals the decoded codepoint, in which case it genuinely is redundant.
...
- **FR-007**: All callers of touch coverage MUST be migrated in the same change — the engine wrapper, Layer C, the studio inventory gate, **and [TouchGallery.tsx](../../packages/studio/src/editors/assignLoop/TouchGallery.tsx)'s direct `computeTouchCoverage` call (`baseTouchCoveredSet`, feeding `collectCompositionMethod`)** — because leaving any one of these four on the unjoined path defeats the fix.
...
- **FR-009**: Reachability MUST treat `K_` ids as always reachable (a physical key exists regardless of the touch layout) and `T_`/`U_` ids as reachable only when carried by a key on a layer reachable from `default`. When no touch layout exists, everything MUST be reachable. **Scope limit:** "reachable" here means **layout-reachable**, not compiler-reachable — this view does not check that a rule's group is itself reachable via `use()` from the entry group, nor that a rule's modifier context corresponds to a layer actually carrying the key. Both are documented limitations, candidates for a later hint-level check, not gaps this feature closes.
...
- **FR-020i**: The grid MUST render **right-to-left layouts mirrored** per the layout's script direction; Home/End and arrow semantics are **logical** (row start/end in reading order, not screen-left/screen-right), and find-by-value result ordering (FR-020e) respects direction. A layout may mix directions per layer (a Latin-numeral layer inside an Arabic keyboard); direction is resolved per layer, not once for the whole layout.
- **FR-020j**: The grid MUST NOT mount every key of every layer: only the active layer's keys are rendered, and layouts beyond a threshold (corpus reality: up to 2,256 keys across 53 layers) MUST window the layer rail and defer offscreen rows. FR-020a bounds Tab stops; this bounds DOM nodes — the same failure class [CharScrollStrip](../../packages/studio/src/editors/assignLoop/parts/CharScrollStrip.tsx)'s `MAX_VISIBLE_CHIPS` was added to prevent.
- **FR-020k**: After a remove/suppress/row action, focus MUST land on the **nearest surviving cell** — the next key in the row, else the previous key, else the row, else the grid container — and MUST NEVER be lost to `document.body` or an unmounted node.
...
- **FR-027a**: When the working copy contains any `RawKmnFragment` (`opaqueFragmentCount > 0`), automatic rule synthesis MUST **downgrade to warn-and-confirm** rather than silently writing a rule: the join cannot prove the absence of an equivalent hand-written rule hidden inside opaque text, and synthesizing anyway risks a permanently-shadowed duplicate. (Mirror language lands in [contracts/key-id-policy.md](contracts/key-id-policy.md) §3.)
...
- **FR-030**: Per-key `layer` (the modifier override) MUST be preserved through parse and emit so that keys legitimately sharing an id within a layer remain distinguishable and addressable. *(Locked-type change — see Needs resolution.)* When `key.layer` is present, it MUST supersede the containing layer in any derived "Sends:" display — the field exists for exactly the keys where the two differ.
- **FR-031**: Every key-level edit MUST promote the key's provenance to `hand-set`, or re-propagation will silently overwrite it. The live promotion path ([touchBehavior.ts](../../packages/studio/src/editors/assignLoop/touchBehavior.ts) `promoteKeyToHandSet`/`promoteOnManualEdit`) is **id-matched** across every platform and layer, and that is intentional for the by-character mechanism flow, where a case-pair companion on another layer should promote with its twin — this FR does not change that helper's semantics. By-key edits MUST instead use a **new address-matched promotion path beside it**, so that a rename cannot miss and same-id keys elsewhere are not promoted incidentally.
...
- **FR-033**: Key-level edits MUST reach the artifact through the **existing single-writer chain**, applied as a raw-JSON overlay, so preview and zip stay identical and Case B byte-preservation (R9) holds for untouched keys on both cases. The overlay applies as a **new step 1.7, immediately after** step 1.6 (`applyTouchKeycapRemovalsToVfs`) in [projectWorkingCopyVfs.ts](../../packages/studio/src/lib/projectWorkingCopyVfs.ts) — step 1.5 (`applyCarveKeycapRemovalsToVfs`) is the text-matched carve pass, a different analog, not this slot. Rename remaps of `deletedTouchKeyIds` happen at **edit-commit time**, through the store's existing delete/restore actions (keeping undo entries consistent), not at emit time — so step ordering cannot see a stale address.
- **FR-033a**: The key edit overlay MUST be an **ordered operation log**, not a keyed snapshot: operations MUST be applied in commit order, and an operation's address MUST resolve against the layout state produced by the operations before it.
- **FR-033b**: **Re-derivation resilience.** Whenever the Case A seed is re-derived (e.g. the author navigates back and changes physical assignments, then returns), overlay operations whose addresses no longer resolve MUST be detected and **reported, never silently dropped** — naming the affected keys and any characters whose placement is lost, offering to discard the orphaned edits or re-place the characters (the FR-062 worklist mechanism). Note: [touchKeyAddress](../../packages/engine/src/pattern-apply/touchKeyAddress.ts)'s silent-miss behavior was designed for the deletion overlay, where a stale address is harmless; for the edit overlay it is not, so resolution failures MUST surface.
- **FR-033c**: **Persistence.** The key edit overlay and the mode selector's state MUST join the persisted working-copy shape ([persistWorkingCopy.ts](../../packages/studio/src/lib/persistWorkingCopy.ts) `PersistedFields`, with the `DurableDraft` version bump that implies), so FR-036b's no-state-discarded guarantee survives a reload — today's persisted set (`deletedTouchKeyIds`, `undoStack`, `touchDraft`, …) does not include it.
...
- **FR-035**: Key-level editing MUST be a **mode of the existing touch step**, not a new step and not a post-lock step. The touch step gains a mode selector (by character / by key); the character-driven walk remains the default. Rationale and the full cost of the alternatives are in [research.md](research.md) R9. The selector MUST be owned by the touch step's existing gallery component, which retains step ownership per research.md R9, and MUST be presented as an APG **tabs** pattern — two tabs, one surface.
...
- **FR-036a**: The mode selector MUST behave as a **view toggle, not a branch**. Switching modes MUST be free and lossless in both directions: no confirmation step, no state discarded, no work to redo. Authors will not know which view they need until they have experimented, so mode choice MUST NOT be a commitment. The selector's state MUST live in the working-copy store beside the existing per-step UI-state precedent (`galleryIntrosSeen`), persisted per FR-033c.
...
- **FR-036c**: **Context MUST carry across the toggle in both directions**, so the two modes read as two lenses on one layout rather than two workflows: switching from a character to by-key MUST select and reveal the key(s) producing that character (or the candidate keys when it is unplaced); switching from a key to by-character MUST land on a character that key produces. The character→key lookup already exists in [enumerateTouchMethodsForChar](../../packages/engine/src/pattern-apply/enumerateTouchMethodsForChar.ts). When a character maps to several keys, the grid MUST select the first in layout order (active platform, family-order layers), badge the remaining producing keys, and offer next/previous cycling among them — the inspector shows one key at a time.
...
- **FR-036e**: **Either mode MUST be able to complete the step.** Continue is gated on coverage (FR-008), never on which view is active; an author MUST never have to switch modes in order to move on. At Continue, both in-progress surfaces — the by-character draft and the key edit overlay — MUST be committed or explicitly resolved; neither is silently discarded.
...
- **FR-038**: The preview MUST be **truthful for every edit this feature adds** — concretely, the preview MUST consume the same working-copy `.kmn` emission the artifact path produces (single-writer, FR-033), whether by extending the touch preview's `vfsTransform` or by sourcing `baseKeyboard` from the emitted working copy; the requirement is **preview/artifact identity**, and `/speckit-plan` chooses the implementation. Absent this, a synthesized `T_`-key rule would not type in the preview and the rule-bearing assignment path would appear to be a silent no-op.
...
- **FR-063**: The studio MUST model a **layer family**: the set of layers that are modifier variants of the same *plane*. A layer id decomposes into `[<plane>-]<modifier-combo>`, with an absent plane meaning the base alphabetic plane — so `default` / `shift` / `caps` / `rightalt-shift` are one family, and `symbol` / `symbol-caps` are another. [comboToTouchLayerId](../../packages/engine/src/pattern-apply/modifierCombos.ts) provides only the forward combo→id direction; **the inverse and the plane concept are new machinery** this feature must add. The decomposition, the freeform-plane fallback (FR-067), the FR-068 property split, and their **test obligations** are contracted in [contracts/layer-families.md](contracts/layer-families.md) — treat it as normative.
...
- **FR-040**: The studio MUST surface, at edit time, the equivalents of Developer's `WARN_TouchLayoutCustomKeyNotDefined` (0x092, with its `nextlayer`/`sp` exemptions), `WARN_TouchLayoutMissingLayer` (0x091), `WARN_TouchLayoutUnidentifiedKey` (0x099), `WARN_TouchLayoutMissingRequiredKeys` (0x093), `WARN_TouchLayoutSpecialLabelOnNormalKey` (0x0A9), and `ERROR_TouchLayoutInvalidIdentifier` (0x05A), plus four checks Developer lacks: duplicate id within a layer, orphan `T_` rule, a modifier key not marked active on its own layer (FR-029d), and a half-done suppression (FR-029c). The invalid-identifier check (0x05A) is a **validity** concern, not hygiene: for author input it MUST be enforced through edit-time rejection (FR-045), and for imported content it MUST route through **Layer A′ import-fidelity** ([layer-a-prime.ts](../../packages/engine/src/validator/layer-a-prime.ts) precedent) — it MUST NOT be added as a Layer C error, since every shipped Layer C check today is warning-only and a first error-severity row is a layering change nobody has signed off on. Edit-time diagnostics and their Layer C siblings MUST share **one underlying implementation** — the join in [contracts/touch-key-rule-join.md](contracts/touch-key-rule-join.md) exists precisely so both surfaces can call it — so the two cannot drift apart.
- **FR-041**: Each diagnostic MUST offer at least one concrete fix action.
- **FR-042**: Diagnostics MUST be computed as **synchronous pure joins with no new timer** (Decision D3 — the 300 ms cycle remains the validation cycle's). Concretely, the touch diagnostics MUST be computed synchronously (e.g. via `useMemo`) from the already-parsed working IR and layout within the existing cycle's products, and composed into the **single aggregated findings surface** — no second store field and no second timer, per the [useValidatorFindings](../../packages/studio/src/hooks/useValidatorFindings.ts) doc's own rule.
- **FR-043**: The corresponding Layer C checks MUST be added as **additional findings emitted by existing 18.x check modules**, adding **no** rows to `criteria.json`. The criteria suite deliberately does not length-test the catalog — `schemas.test.ts`/`types.test.ts` assert schema-validity and the four-band partition invariant, not cardinality; the row count is descriptive and expected to grow (see the Contract source-of-truth chain in [CLAUDE.md](../../CLAUDE.md)). The "no new rows" decision holds for a different, sound reason: `CriteriaBands.lintRuleId` is a singular, 1:1 field, so a new diagnostic code is a **check-level** addition — documented in the check module's own file — rather than a criteria-row addition hung off a row whose existing description does not cover it.
...
- **Key edit overlay** — an **ordered log of committed key-level operations**, replayed in commit sequence against the layout state each prior operation produced (a later op resolves against what came before it, so rename-then-add is coherent by construction), applied as a raw-JSON pass over the derived layout; the unit of undo and of persistence.
...
- **SC-004**: An author can assign a character to an existing `T_*` key and see it type in the live preview, using only the keyboard, in at most 12 discrete keyboard actions beyond navigation to the touch stage, with no modal detour (the US2 AS5 path: Tab, arrows, Enter, type, Enter). "Under two minutes" holds only as narrative framing, not the measured assertion.
...
- **SC-006**: On an import-adapt walk, editing one key leaves **untouched files** byte-identical to the shipped source, and within the touched `.keyman-touch-layout` file, every untouched key and platform-level field is **structurally identical** to the source. The existing raw-JSON pass re-serializes the whole file, so formatting normalizes — a known limitation; byte-level patch-minimization is out of scope.
...
- **SC-009**: The new grid passes an automated accessibility scan with no serious violations and is fully operable with no pointer events — enforced by the existing `expectNoSeriousAxeViolations` helper ([e2e/helpers/axe.ts](../../packages/studio/e2e/helpers/axe.ts), spec 056 FR-003), with a dedicated pass on the grid in roving-tabindex state.
- **SC-010**: No new debounce timer exists anywhere in the feature — enforced by a fake-timer behavioral spec in the [useKeyboardArtifact.test.ts](../../packages/studio/src/hooks/useKeyboardArtifact.test.ts) mold, asserting the new diagnostics resolve within the existing 300 ms cycle with no additional timer callback.
- **SC-011**: Switching modes N times in any order loses no state in either direction, and the shared progress figures (FR-036d) never disagree between views — enforced by a vitest suite on the store plus one Playwright toggle scenario.
...
- The corpus at `../keyboards` (the `keyboard-studio/keyboards` fork) is the calibration source; the figures in this spec are measured against it and are expected to drift with the pin. **The corpus-wide aggregate figures (the Corpus calibration table above) are narrative calibration only and MUST NOT be asserted by tests.** Only the two named Cameroon canaries — `sil_cameroon_qwerty` and `sil_cameroon_azerty` — are pinned fixtures, and tests referencing them MUST skip-if-absent.
