...
- [x] T001 Confirm working tree is on branch `km/qu-026-survey-session-store` and clean of
...
- [x] T003 Re-verify the exact `ActiveStepId` union members at
...
- [x] T004 [US1] Create `packages/studio/src/stores/surveySessionStore.ts` mirroring the
...
- [x] T005 [US1] Create `packages/studio/src/stores/surveySessionStore.test.ts` covering
  ...
  store between cases.
- [x] T006 [US1] Run `pnpm --filter @keyboard-studio/studio test src/stores/surveySessionStore.test.ts`
...
- [x] T007 [US2] In `packages/studio/src/StudioShell.tsx`: delete the local `useState` for
...
- [x] T008 [US2] Delete `selectedTrackRef` and its sync effect; change the memoised
  `onInstantiate` to read `useSurveySessionStore.getState().selectedTrack` (research D-R3/D-R5).
- [x] T009 [US2] Route every forward transition through `advance(nextId)` (was
  ...
  store actions.
- [x] T010 [US2] Rewire `handleStartOver` to call `session.reset()` first, then
...
- [x] T011 Run the three parity oracles and confirm they pass **without any source edit**:
...
- [x] T012 Run the full gate set: `pnpm --filter @keyboard-studio/studio typecheck`,
...
- [x] T013 Final scan of the diff: no leftover `setActiveStepId`/`selectedTrackRef` references;
