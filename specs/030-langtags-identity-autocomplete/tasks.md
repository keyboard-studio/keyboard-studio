...
- [ ] T031 Full gate: `pnpm typecheck && pnpm -r test && pnpm lint && pnpm depcruise` (all green modulo the known Node-26-local jsdom/crypto env failures that pass on CI Node 22). **BLOCKED (2026-07-19) by a pre-existing local-only failure that is out of 030 scope — see FOLLOW-UP below. Gate is green on CI; kept unchecked so a real local red is not marked green.**

> **FOLLOW-UP (blocks T031, out of 030 scope) — `classifyRemovalCapabilities` C2b canary vs Keyman 17+ corpus drift.**
> Running the full gate locally, `packages/engine/src/recognizer/classifyRemovalCapabilities.test.ts` C2b fails deterministically: `basic_kbdus` K_A..K_Z rules classify `not-removable:unknown`, but the canary expects `removable:simple`. Root cause is **corpus drift, not a 030 change**: the sibling `../keyboards/release/basic/basic_kbdus/source/basic_kbdus.kmn` was updated to Keyman 17+ project format (corpus commit 9ca2b3ced, 2026-06-15) and now carries a `store(&CasedKeys) [K_A]..[K_Z]`, which the canary predates. The recognizer/classifier code is byte-identical to `main` (the only branch commit is spec 040, touching `placement/filters.ts` + facet-index — nothing in `recognizer/`). The test is `it.skipIf(!kbdusExists)`, so on CI (no external corpus checked out) it **skips** and the gate is green — matching this task's "green on CI" note.
> **Decision needed (km-keyman / km-strategy):** is a direct-key letter rule under a `&CasedKeys` store still `removable:simple`? Then either update the C2b canary's expectation for the Keyman 17+ `&CasedKeys` shape, or teach the classifier to handle it. Once resolved (or the caveat is deliberately widened to include this canary), flip T031.
> A secondary, non-blocking observation: `packages/glottolog` `codegen-determinism.test.ts` failed on the *first* full-gate run of a session but passed on isolated re-run and on the second full run; regenerating produces a byte-identical index (`git diff --numstat` = 0). Looks like a first-invocation artifact of the codegen's write-only-on-change guard, not a persistent blocker.
