...
- **NEW** test `packages/studio/src/dashboard/buildListPhaseResult.test.ts` (co-located with the spec-019 `prefillRouting.test.ts` oracle per §2.5) — the §2.5 `SurveyPhaseResult` oracle. **[Per U1]** complete the build-list path for a **fixed input sequence** and assert the produced `SurveyPhaseResult`'s `confirmedInventory` union via `mergePhaseResults` against a **CONCRETE committed inline snapshot** (`toMatchInlineSnapshot`, mirroring `prefillRouting.test.ts`) of the POST-`mergePhaseResults` deduped / NFC-normalised union, and assert `KeyboardIR` is **not** written by the build-list path.
...
5. **Guardrail confirmation:** run the spec-016 drift guardrail with `pb_build_list` resolving as the build-list branch drill-down; it is checked in the **question** graph (the boundary-crossing step — reachable-from-flow-entry via `resolveNext` over `next`/`FlowGotoRule[]`, §2.2(b)), so the bijection holds. **[Amended per I1]** The reachable node is the registry anchor `pb_discovery_intro` (the IntroChooser gate); `pb_build_list` is a declared-only drill-down descriptor anchored to it, which must resolve as a rendered drill-down (no orphan, no uncovered step) — it is NOT itself a reachable `questionRegistry` id (per the merged `drillDownDeclarations.ts` / `pb_build_list.test.ts`).
...
- **Drift guardrail stays green:** **[per I1]** the registry anchor `pb_discovery_intro` (the IntroChooser gate) is the reachable question-graph node; the `pb_build_list` drill-down descriptor hung off it resolves as a rendered drill-down, so the spec-016 bijection holds in the **question** graph (the boundary-crossing step) (FR-010/SC-006).
...
- **`SurveyPhaseResult` oracle (FR-009):** complete the build-list path for a **fixed input sequence**; assert the produced `SurveyPhaseResult` (`confirmedInventory` union via `mergePhaseResults`) against a **CONCRETE committed inline snapshot** (`toMatchInlineSnapshot`, mirroring spec 019's `prefillRouting.test.ts`) of the POST-`mergePhaseResults` deduped / NFC-normalised union; assert `KeyboardIR` is **not** written by the build-list path. **[Per U1]** the fixed input includes a duplicate, a whitespace-only token, and an NFD entry so the snapshot pins the dedup + NFC + empties-dropped contract. (No emit-byte or flow-routing comparison — build-list writes `confirmedInventory`, NOT `KeyboardIR`.)
...
- **Drift guardrail (spec 016):** green with the build-list branch checked in the **question** graph (the boundary-crossing step, §2.2(b)) — **[per I1]** the registry anchor `pb_discovery_intro` is the reachable node, with the `pb_build_list` drill-down hung off it resolving as a rendered drill-down (no orphan, no uncovered step).
