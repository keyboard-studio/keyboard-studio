...
### Session 2026-06-29 — reconciliation against what landed (I1 / C1 / U1)

`/speckit-analyze` found that the original draft's framing of `pb_build_list` as "a reachable question-graph node" was **wrong** against what specs 016 and 017 actually ratified. The following three reconciliations make this spec self-consistent with the merged code; they change no Phase-1 invariant.

- **I1 (node naming — the reachable/rendered node is `pb_discovery_intro`, NOT `pb_build_list`).** Spec 016's drift guardrail and spec 017's [drillDownDeclarations.ts](../../packages/studio/src/survey/questions/drillDownDeclarations.ts) + merged [tests/survey/questions/b/pb_build_list.test.ts](../../packages/studio/tests/survey/questions/b/pb_build_list.test.ts) (lines 86-92, 140-147) ratified that **`pb_build_list` is a DECLARED-ONLY drill-down descriptor — NOT a `questionRegistry` id and NOT itself reachable**. The reachable/rendered node at the discovery-method gate is `pb_discovery_intro` (the IntroChooser, [PhaseB.tsx](../../packages/studio/src/survey/PhaseB.tsx) — `pb_discovery_intro` is the modular-flow question that renders the chooser), and `pb_build_list` is the build-list-branch drill-down descriptor **anchored to it** via `registryKey: "pb_discovery_intro"`. FR-001 / FR-010 / SC-006 and tasks T009 / T018 are amended accordingly: the reachable question-graph node is the registry anchor `pb_discovery_intro`; `pb_build_list` is the drill-down hung off it (it must have a rendered drill-down, not be itself a reachable node). This mirrors exactly how spec 019 ([prefillRouting.test.ts](../../packages/studio/src/dashboard/prefillRouting.test.ts)) and the merged `pb_build_list.test.ts:140-147` already phrase it.
- **C1 (map-node / invariant assertions stay strictly ADDITIVE).** The merged spec-017 [tests/survey/questions/b/pb_build_list.test.ts](../../packages/studio/tests/survey/questions/b/pb_build_list.test.ts) already ratifies: drill-down placement under `characters` (`underNodeId === CHARACTERS_NODE_ID`), the registry-key anchor (`registryKey === "pb_discovery_intro"`), `writes: []`, output on `confirmedInventory` (`{ kind: "phase-result-field", field: "confirmedInventory" }`), no `header.script`, and satisfiability of `header.bcp47`. Tasks T009–T011 / T016 **reference that test rather than re-asserting it**; this spec's new tests assert only what is genuinely additive — the descriptor surfacing on the map projection (the drill-down hangs under the projected `characters` node) and the runtime `SurveyPhaseResult` oracle.
- **U1 (the `SurveyPhaseResult` oracle is a CONCRETE committed baseline).** The new oracle (T006–T008, FR-009) is an **inline snapshot** of the produced `confirmedInventory` union for a **fixed input sequence** (mirroring spec 019's `prefillRouting.test.ts` `toMatchInlineSnapshot` approach), asserting the **POST-`mergePhaseResults` deduped / NFC-normalised union** — NOT a raw per-phase `confirmedInventory` array. The fixed input deliberately includes a duplicate, an empty/whitespace token, and an NFD entry so the snapshot pins the dedup + NFC-NFC normalisation + empties-dropped behaviour `mergePhaseResults` applies. This is stated in the test header and in FR-009 below.
...
**Independent Test**: Run the drift guardrail (spec 016) with `pb_build_list` resolving as the build-list branch drill-down; confirm green — **[per I1]** the registry anchor `pb_discovery_intro` (the IntroChooser gate) is the reachable question-graph node, and the `pb_build_list` drill-down hung off it resolves as a rendered drill-down (no orphan, no uncovered step). Run `pnpm typecheck`, the studio + contracts vitest suites, and `pnpm depcruise`; confirm green.

**Acceptance Scenarios**:

1. **Given** `pb_build_list` resolving as the build-list branch drill-down, **When** the drift guardrail (spec 016) runs, **Then** it stays green — **[per I1]** its registry anchor `pb_discovery_intro` (the IntroChooser gate) is checked as the reachable node in the **question** graph (the boundary-crossing step, §2.2(b)) and the `pb_build_list` drill-down hung off it has a rendered drill-down (no orphan, no uncovered step). `pb_build_list` is a declared-only descriptor, not itself a reachable node.
...
- **FR-001**: The default Phase B `BuildListView` (`PhaseB.tsx:535`, used ~`692`) MUST resolve as the **build-list branch** drill-down descriptor anchored to the **mandatory** IntroChooser discovery-method gate. **[Amended per I1]** The reachable/rendered node at that gate is `pb_discovery_intro` (the IntroChooser question that renders the chooser); `pb_build_list` is the build-list-branch drill-down descriptor **anchored to it** via `registryKey: "pb_discovery_intro"` (a declared-only descriptor, NOT a `questionRegistry` id and NOT itself reachable — per the merged `drillDownDeclarations.ts` and `pb_build_list.test.ts:86-92`). This spec relies on the spec-017 `pb_build_list` drill-down declaration being projected by the map adapter as a drill-down under the opaque `characters` node; it MUST NOT add a top-level manifest entry and MUST NOT promote `pb_build_list` to a `questionRegistry` id.
...
- **FR-009**: A **`SurveyPhaseResult` oracle** test (migration-plan §2.5, `SurveyPhaseResult`-writing surfaces) MUST assert that the produced `SurveyPhaseResult` — the `confirmedInventory` union via `mergePhaseResults` — is **deep-equal before/after** this spec. Build-list writes `confirmedInventory`, NOT `KeyboardIR`, so the oracle is the phase-result deep-equal, not an emit-byte or flow-routing comparison. **[Amended per U1]** The oracle is a **CONCRETE committed baseline**: an **inline snapshot** (`toMatchInlineSnapshot`, mirroring spec 019's `prefillRouting.test.ts`) of the **POST-`mergePhaseResults` deduped / NFC-normalised `confirmedInventory` union** for a **fixed input sequence** — NOT a raw per-phase `confirmedInventory` array. The fixed input includes a duplicate, a whitespace-only token, and an NFD entry, so the committed snapshot pins the dedup + NFC normalisation + empties-dropped contract `mergePhaseResults` applies.

**Guardrail & gate**

- **FR-010**: The drift guardrail (spec 016) MUST stay **green** with the build-list branch checked in the **question** graph (the boundary-crossing step, §2.2(b)). **[Amended per I1]** `pb_build_list` is NOT itself a reachable question-graph node — it is a declared-only drill-down descriptor; its **registry anchor `pb_discovery_intro` (the IntroChooser gate)** is the reachable question-graph node, and the `pb_build_list` drill-down hung off it MUST resolve as a rendered drill-down (no orphan, no uncovered step). This matches the merged `pb_build_list.test.ts:140-147` reachability assertion.
...
- **SC-006**: The drift guardrail (spec 016) stays **green** with the build-list branch checked in the **question** graph (the boundary-crossing step). **[Amended per I1]** The reachable question-graph node is the registry anchor `pb_discovery_intro` (the IntroChooser gate); the `pb_build_list` drill-down hung off it resolves as a rendered drill-down (no orphan / uncovered step). `pb_build_list` is a declared-only descriptor, not itself a reachable node.
