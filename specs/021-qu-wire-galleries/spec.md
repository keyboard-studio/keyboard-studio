...
**Status**: Implemented (all FRs/SCs) — the render-layer wiring landed and merged to `main`. Re-verified 2026-08-20: `registerEditorSteps.ts` still declares `carve`/`mechanisms` (`surface:"physical"`)/`touch` (`surface`/lock via `manifest.ts`) with no second declaration; `reducer.ts`'s `MECHANISMS_STEP_ID` branch still fires `deps.lockDesktop()` unconditionally with the re-propagation add-on gated behind `isMutateSeamEnabled()` immediately below (line numbers have drifted from this spec's citations — later specs, notably 014's mutate seam, added code above these branches — but the structure and invariants are unchanged); `dashboard/driftGuardrail.test.ts` and `steps/reducer.test.ts`'s R1/R2 describe blocks cover these paths. **Two of the three flows (mechanisms / physical R1 and touch R2) are KNOWN-GOOD REFERENCE flows that MUST NOT regress.**
