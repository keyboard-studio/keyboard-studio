...
- **FR-003**: Exception-site enumeration MUST be **deterministically recomputable** at build time (spec 037 rule); the committed index stores the **summary** (value + consistency + cause-tag counts), not the per-site enumeration (spec 070 rule).
