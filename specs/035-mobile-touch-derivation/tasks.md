...
**Input**: Design documents from `specs/035-mobile-touch-derivation/` — [plan.md](plan.md), [spec.md](spec.md), [research.md](research.md) (R1–R12, post-amendment), [data-model.md](data-model.md), [contracts/](contracts/), [quickstart.md](quickstart.md)
...
- [ ] T001 Record a pre-change baseline: run `pnpm --filter @keyboard-studio/engine test src/scaffolder/scaffoldTouchLayout.test.ts src/pattern-apply/applyTouchAssignments.test.ts src/pattern-apply/applyTouchAssignmentsToRawJson.test.ts src/codec/parse-touch.test.ts` and `pnpm --filter @keyboard-studio/studio test src/lib/buildTouchLayoutJson.test.ts src/steps/advance.test.ts src/editors/assignLoop/TouchGallery.test.tsx`; note any pre-existing Windows-checkout failures (CRLF/golden — known, unrelated) in the PR description so they are not chased later
...
- [ ] T024 Reconcile the spec-014 seam (seed-derivation contract "Adjacent-seam note"): `packages/studio/src/steps/repropagate.ts` + `packages/studio/src/editors/touchSuggest/touchSuggest.ts` must not form a second physical→touch propagation path alongside the replay — either share placement logic or document the explicit ordering rule in both module docstrings; `packages/studio/src/lib/serializeWorkingCopy.flagParity.test.ts` must stay green (flag-off byte-parity)
...
- **Phase 2 (Foundational)**: after T001. Internal ordering: T002/T003/T004 in parallel → T005/T006/T007 in parallel; T008 after T004; T009 after T008; T010 independent [can run parallel with engine tasks]; T011 after T002+T003; T012 after T010+T011; T013 independent of engine tasks; T14 after T013
