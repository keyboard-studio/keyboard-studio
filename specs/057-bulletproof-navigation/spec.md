...
- [specs/072-my-keyboards/](../072-my-keyboards/) — the per-project draft scheme and its display-label precedence. The footer's project name reuses that derivation rather than forking a fourth definition.
...
- **Q6 — Is the footer shown on every tab, including Welcome and Profile?** ~~**Recommended default:** shown on every tab once a project exists; hidden on Welcome (no project yet) and wherever there is no project to name.~~ **Revised 2026-08-05 (author):** shown from the FIRST question onward, on every tab; hidden only before the journey starts. The original default gated the strip on a project NAME, whose earliest source is filled at instantiation — i.e. at base selection. But the identity-lite battery runs before that, and its answers (English name, autonym, language code, author attribution) reach the emitted `.kps`. The strip was therefore hidden for exactly the stretch in which the author had already made several recordable decisions and most needed to see where they were. The project name is a separate matter from the strip: it is omitted, not placeholdered, until one exists (FR-041 forbids inventing a fourth derivation, and the identity-lite result is an answer about the language, not a name for the project).
...
5. **Given** the journey has not started, **When** the author is on the Welcome screen, **Then** the footer is absent rather than showing an empty shell (per Q6, revised 2026-08-05). Conversely, given the author is on the very first identity question with no base keyboard chosen, the footer IS present — with the dot row and no project name.
5a. **Given** the author is in a gallery (carve, mechanisms, touch) or on the Phase B "add your whole alphabet" page, **When** they look at the footer, **Then** that page contributes exactly ONE dot — never one per character (author's call, 2026-08-05: each gallery has its own in-page navigation to the character wanted, which the per-letter dots merely duplicated). A question battery such as the marks series still contributes one dot per question.
5b. **Given** the author has jumped back to an earlier stage, **When** they activate a dot for a stage they had already completed, **Then** the jump ARRIVES — a stage is refused only when they have genuinely never reached it. Neither leg of the round trip loses position, in-progress answers, or gallery state.
...
- **FR-007**: Re-entering the characters step at its build-list substage MUST NOT clear the Phase B draft alphabet. Clearing remains tied to a genuine prefill → build-list transition, not to a remount (D-4).
  **Amendment (spec 079).** Narrowed: clearing is tied not to "a genuine prefill → build-list transition" but to a **change** in the language, script or base the prefill confirms (079 FR-020). Passing through the prefill confirmation unchanged is not a change, and a real change carries the author's own edits over (079 FR-015).
...
- **FR-040**: A narrow footer MUST be present from the author's first question onward, on every tab (Q6, revised 2026-08-05), styled from the existing theme tokens rather than new hard-coded colours, and MUST NOT materially reduce the vertical space available to the walk. It MUST be absent — not an empty shell — before the journey starts.
...
- **FR-042**: The footer's dot row MUST be a **whole-journey** strip, not a history: it shows where the author has been, where they are, and what is still to come. It carries three classes of mark, in journey order (resolved 2026-08-03):
  ...
  **Amendment (spec 079).** Three changes, per [spec 079 Amendments](../079-survey-answer-persistence/spec.md#amendments-to-existing-specs):
  - *Grain.* The strip has two tiers. Every section (manifest step) is one large mark, and the section the author is in expands in place into small question marks, one per author-facing screen (one Next), never one per recorded answer (079 FR-060). This pins FR-042/FR-049's grain and covers per-question marks inside multi-screen steps.
  - *Recording.* "A dot appears when its step completes… the decision-recording path is NOT changed to commit per question" is superseded. Answers are recorded at each Next (079 FR-040), so a question mark becomes a completed mark at its own Next.
  - *Badge state.* Any mark may carry a "work to do" badge (079 FR-017), alongside its existing class and position cues. This spec's accessibility (FR-043…FR-046) and jump (FR-045) rules apply to it unchanged.
