...
- **Offer scope is a Content call**: what the accept action covers (Engine's proposed default: `main` only, with `auxiliary`/`punctuation`/`numbers` offered separately in their 047 sections) and the offer's wording are Content-owned per the §12/§13 split, and need Content sign-off before FR-016 is implemented.
