...
- **FR-006**: A **descending** range (first endpoint > second) MUST NOT be expanded in the wrong direction or as an empty set silently. [NEEDS CLARIFICATION: preferred fail-safe — (a) preserve the store opaque with a diagnostic reason, (b) treat as a compile-invalid construct surfaced by the validator, or (c) expand descending as an explicit reverse range if Keyman semantics allow it? Confirm against kmcmplib behaviour.]
- **FR-007**: Expansion MUST be bounded and deterministic for large ranges. [NEEDS CLARIFICATION: is any upper bound on a single range's cardinality required (memory/perf), or is the natural Unicode ceiling sufficient? Corpus max observed ≈ 800 cp (`U+E000 .. U+E317`).]
- **FR-008**: The change MUST preserve the codec's round-trip contract for range stores at the level spec 041 requires (semantic round-trip; byte-identical is explicitly out of scope). Re-parsing emitted output MUST yield the same codepoint set. [NEEDS CLARIFICATION: should emit **re-collapse** an expanded run back to `X .. Y` notation to keep authored `.kmn` compact and human-legible, or is emitting the expanded literal list acceptable? This drives the IR-representation decision in Key Entities.]
- **FR-009**: The feature's scope is **store bodies**. [NEEDS CLARIFICATION: do range operators occur in rule context/output positions in the corpus, and must they be handled here, or is that explicitly deferred?]
- **FR-010**: The feature MUST leave the existing `smp-literal` opaque handling correct for the constructs it legitimately covers (astral literals the typed IR still cannot represent), changing behaviour only for the range case. [NEEDS CLARIFICATION: does lifting SMP-opaqueness for range stores imply, or require, also representing standalone astral `U+XXXXX` store items as typed char items? Resolve the consistency between "astral range expands to char items" and "astral singleton is opaque".]
...
- **Store item (IR)** — the parsed unit of a store body. The design decision this spec surfaces: **(A)** expand a range eagerly into individual `{kind:"char"}` items (no contract change; simplest for consumers; emit either re-collapses or lists), or **(B)** introduce a new preserved `{kind:"range", from, to}` store-item variant (a **locked-contract change** to `packages/contracts` → per the revision policy needs a `@keyboard-studio/contracts` major bump + joint engine+content session; better round-trip fidelity; consumers must learn to expand it). [NEEDS CLARIFICATION: choose A or B. Recommendation to evaluate in `/speckit-plan`: **A** if `buildProducedSet` and friends only ever need the expanded set and compact emit can be achieved by run-collapse; **B** only if a downstream consumer must distinguish an authored range from an expanded list.]
...
- Range notation semantics follow Keyman/kmcmplib: `X .. Y` is an inclusive, ascending run of codepoints; endpoints may be `U+` or quoted literals. The exact descending/degenerate behaviour is to be confirmed against kmcmplib (FR-006).
...
- If the IR-representation decision is **B** (new store-item kind), the contracts major-version bump + joint session is accepted as part of this feature's cost; if **A**, no contract change is needed.
