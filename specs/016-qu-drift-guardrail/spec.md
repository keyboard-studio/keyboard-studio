...
These are surfaced as `[NEEDS DECISION]` and resolved during `/speckit-plan`, not invented here:

- **D1 — test location & import boundary.** Where the test physically lives, and whether it imports `buildManifestStepGraph` directly. `completeness.ts` DELIBERATELY avoids that import to prevent a circular dependency (`completeness.ts:526`: `completeness.ts → buildStepGraph.ts → manifest.ts → registerEditorSteps → editors/ → stores/ → completeness.ts`). A test file does not sit on that cycle, so a test MAY import `buildManifestStepGraph` directly — but the boundary must be confirmed against `pnpm depcruise` during planning. See FR-009 / [NEEDS DECISION: D1].
- **D2 — representation of "the node set the dashboard actually renders."** Re-run the 015 adapter in-test vs snapshot the `DashboardView` graph output. See FR-002 / [NEEDS DECISION: D2].
...
- **FR-002**: The guardrail MUST obtain "the node set the dashboard actually renders" in a way that reflects the real 015 render path (the adapter + drill-downs), not a re-derivation that could itself drift. **[NEEDS DECISION: D2 — re-run the 015 adapter in-test vs snapshot the `DashboardView` graph output.]**
...
- **FR-009**: The guardrail MUST be **distinct from** the pre-existing tautological test at `buildStepGraph.test.ts:323-356` (the C8/C9 block) and MUST catch the §1 drift that test cannot. It MUST NOT reuse or re-assert the manifest ↔ `buildManifestStepGraph` identity that test already covers. The guardrail's physical location and whether it imports `buildManifestStepGraph` directly (which `completeness.ts` avoids for a circular-dependency reason that does not bind a test file, `completeness.ts:526`) is **[NEEDS DECISION: D1]**, to be resolved against `pnpm depcruise` during planning.
- **FR-010**: The guardrail MUST be **co-located** with the existing manifest-shape guards (`validateManifestShape()` M2–M6, incl. M4b — `manifest.test.ts`) and the completeness checks C1–C7 (`dashboard/completeness.ts` / `completeness.test.ts`), in the established test tree.
- **FR-011**: This feature MUST introduce **no new write routing**, **no contracts bump**, and **no flag flip**; observable runtime/IR/render behavior MUST remain **byte-identical**. The only artifact is the new test (plus, if D2 resolves that way, a tiny test-only helper to expose the rendered node set).
