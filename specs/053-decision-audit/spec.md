...
- **Aggregation boundary is the step**: editor activity is summarised when a step completes. Activity in a step the author has not yet completed is not yet an entry.
  **Amendment (spec 079).** The capture point for *survey answers* moves from step completion to **each Next** within a step (079 FR-040): a multi-question step records per screen as the author advances. The append-only model and the supersession chains (FR-015) are unchanged. An intermediate Next in a multi-screen step is a capture boundary whose net diff is empty, so its entries carry the `{state: "none"}` impact; the step's final Next carries the whole series' diff (079 research R-05, plan risk 2). Editor-activity aggregation per step is unchanged.
