...
- **Duplicated composed-unit → one-mark-shorter-pair logic.** `mark-guards.ts`'s `buildUnwrap` and `context-variants.ts`'s `addBackspaceUnwrap` now both reimplement the same NFD-slice/NFC-recompose pairing independently. research.md's Phase 0 decision to avoid consolidating onto `nfcPostureOfInventory` no longer fully covers this — that function is not actually called by either `context-variants.ts` call site (needs a `ConfirmedAlphabet` neither has). A shared pure helper (candidate home: `pattern-apply/ir-insert.ts`) would remove the duplication without reopening spec 071.
