...
- **Studio wiring is a deliberate follow-up, not dropped scope.** `computeContextTolerance` / `createContextToleranceMigrationRule` / `proposeContextVariants` are built and tested at the engine/contracts/lint layer (every call site in this PR's diff is a test), but nothing yet calls them from `useFacetTransform.ts` or the studio's real compile-gate — this PR lands the engine-layer capability; wiring it into an end-user-reachable path is tracked as separate follow-up work, not an oversight.
