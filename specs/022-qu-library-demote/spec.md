...
**Status**: Implemented (scope reduced by amendment) — FR-001 MET, FR-002 STRUCK, FR-008 DEFERRED (see amendment notes above; a deliberate scope cut, not a shortfall). Phase 1, spec #8 (the final Phase-1 spec) of the Question Unification migration ([docs/design-notes/question-unification-migration-plan.md](../../docs/design-notes/question-unification-migration-plan.md) §5). Full non-identity Phase A demoted to registry-only reserve, rendered via `computeReserveNodes`, backed by `noDeleteGuardrail.test.ts` and `strategyAxisLock.test.ts`; the `pb_*` battery stays live/reachable per the amendment. Retroactively verified 2026-08-20.
