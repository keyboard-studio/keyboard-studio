...
- [ ] **T007** Take the `pb_*` battery **off the default spine** in `content/flows/phase_b_characters.modular.yaml`: remove it from the active **default** ordering while keeping it on the **step-by-step branch reachable via the mandatory IntroChooser gate** (the battery stays reached via `resolveNext` over the gate's `FlowGotoRule[]`). It must render as a reserve node **where it is not on the active branch** and stay **reachable** (FR-002). Do NOT make it unreachable; do NOT introduce an auto-default skipping the gate.
- [ ] **T008** Take the **full non-identity Phase A** out of the default ordering in `content/flows/phase_a_identity.modular.yaml` so all 15 identity + 15 `provenance_*` modules become reserve (FR-001). `identity_lite` is canonical. Do NOT wire `PhaseA` into `StudioShell` (`StudioShell.tsx:18`) — the revival alternative is rejected (FR-001/FR-011).
- [ ] **T009** Confirm the membership edit did **not** touch any module `.ts` file or any sub-registry entry (`registry.a.ts` / `registry.b.ts` / `registry.ts`) — demotion is a YAML-membership change only (no-delete; FR-004).
