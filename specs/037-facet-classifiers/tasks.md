...
## ⚠️ Blocking dependency on spec 036 (READ FIRST)

037 owns **algorithms**; 036 owns the **harness** they plug into (the `utilities/facet-index/` build loop, the pinned UCD slim-lookup codegen, the Entity 2 index-record shape + `facet-index-lint`, and `content/keyboard-facets/script.yaml`). As of this branch (`km/source-facets`) that harness is **not present on disk** and not on `main` — 036 is spec'd (branch `km/facet-index`) but unimplemented. Phase 1 begins by reconciling this: either 036 has landed (verify) or 037 stands up the **minimal** harness surface it needs, explicitly flagged so it can be reconciled with 036 at merge. Do not silently duplicate 036's work — coordinate via the lockout registry (concurrent 036/039 work is active).
...
- [ ] T001 Reconcile the 036 harness: check for `utilities/facet-index/build-index.ts`, `utilities/facet-index/ucd/generated/scriptLookup.ts` (or `lib/ucd/` + `codegen-ucd.mjs`), and `content/keyboard-facets/script.yaml`. Record in a short note at the top of `utilities/facet-index/classifiers/README.md` whether each is present (036 landed) or must be stood up here as a flagged stub for later reconciliation with 036.
- [ ] T002 Create the classifier subtree `utilities/facet-index/classifiers/` with a `README.md` stating the engine/content ownership split (Article VI: algorithms here, definitions under `content/keyboard-facets/`) and the lockout coordination note.
- [ ] T003 [P] If absent from 036, stand up the minimal pinned UCD slim lookup at `utilities/facet-index/ucd/generated/scriptLookup.ts` exposing `scriptOf(cp)`, `scriptExtOf(cp)`, `blockOf(cp)` (Unicode 17.0.0; codepoint miss ⇒ `Zzzz`, never throw; `scriptExtOf` sets canonically sorted). Flag as 036-reconcile if stubbed. (data-model Entity 2)
- [ ] T004 [P] Confirm `tsx` + `vitest` resolve for `utilities/facet-index/` (standalone tool, out of `pnpm -r`); add/verify its local tsconfig so `pnpm exec vitest run utilities/facet-index/classifiers/*.test.ts` works.
...
- [ ] T005 Implement the shared `Classifier` interface + `Categorization` + `ClassifierInputs` + `Archetype` types in `utilities/facet-index/classifiers/classifier.ts`, verbatim to [contracts/classifier.contract.md](contracts/classifier.contract.md) (C1–C8 obligations). Pure/deterministic contract; no `Date.now()`/`Math.random()`.
...
- [ ] T007 [P] Implement the `ReferenceData` loader in `utilities/facet-index/classifiers/refs.ts`: wraps the UCD slim lookup (`scriptOf`/`scriptExtOf`/`blockOf`) and `getLanguageDefaults` (langtags tier-3 default script). Pins recorded for the 036 manifest (FR-004).
- [ ] T008 Wire the shared contract into the build loop: `build-index.ts` iterates registered classifiers, calls `classify(inputs, refs)`, and writes each `Categorization` into the 036 Entity 2 record for its facet. Registry keyed by `classifier.id` = `derivation.classifierId`.
...
- [ ] T011 [US1] Implement the content-derived histogram in `utilities/facet-index/classifiers/script.ts`: iterate `producedSet` (NFC scalars), map each via `scriptOf`/`scriptExtOf`, exclude `Zyyy`/`Zinh`/`Zzzz` from the denominator (FR-008), give **full weight** to every `Script_Extensions` member, normalize by weighted total, sort keys; `0/0 → 0` (FR-007–FR-009). Count `Zzzz` unknowns distinctly in `notes`.
...
- [ ] T013 [US1] Implement the Latin sub-profile (FR-010) in `script.ts`: when `value === "Latn"`, derive `subProfile.latin ∈ {plain, extended, ipa}` from block membership (IPA/Phonetic Extensions ranges) against a Latin-specific evidence floor; label it a hint, not an orthography claim.
- [ ] T014 [US1] Implement the fallback chain + confidence/outcome classing in `script.ts`: content (≥10 concrete chars AND ≥50% coverage) → declared script subtags → `getLanguageDefaults().defaultScript` → undetermined; set `confidenceClass` (dominant ≥0.80 `confident`, else `mixed`, no evidence `undetermined`), `provenanceTier`, `evidenceSize`, `analyzedCoverage = 1 − opaqueShare`, `analysisOutcome` (fully/partially/fallback-only). (FR-011, data-model Entity 3a, C4–C7)
- [ ] T015 [US1] Ship the `content/keyboard-facets/script.yaml` definition (histogram over closed ISO 15924 set, `derivation.classifierId: script`, fallback chain) **if 036 has not shipped it**; otherwise verify 037's classifier `id` matches its `derivation.classifierId`. **Content-team-owned** (Article VI).
- [ ] T016 [US1] Add any newly-cited fixture keyboards to the phonebook [docs/keyboard-index.md](../../docs/keyboard-index.md) (mandatory — read each keyboard's `.kps` for name/BCP47/author).
...
- [ ] T021 [P] [US3] Fixture test `utilities/facet-index/classifiers/target-mix.test.ts` (FR-006, SC-004): desktop-only (`<Targets>` absent) → `value:["desktop"]`, `provenanceTier:"default-fallback"`; touch-layout sibling present → `value` includes `"touch"` even if declaration omits it, `notes` mismatch flag (artifact outranks declaration, FR-014 AC1); `web`-declaring → includes `"web"`; `&TARGETS 'any'` → `["desktop","touch","web"]` (sentinel expanded).

### Implementation for User Story 3

- [ ] T022 [US3] Implement `utilities/facet-index/classifiers/target-mix.ts`: union of `.kps <Targets>` (enum-validated, defaults to `windows`/desktop when absent — AC2), `.kmn &TARGETS` (raw; `'any'` → all device classes), and touch-layout artifact presence (`LAYOUTFILE` sibling via `siblingPresent`); map `KeymanPlatformTarget` → {desktop, touch, web}; per-member per-source provenance in `distribution`; flag declaration/artifact mismatch in `notes` (FR-014, data-model Entity 3c, C5).
- [ ] T023 [US3] Ship `content/keyboard-facets/target-mix.yaml` (set over {desktop, touch, web}, `open:false`, `feedsSessionFacets: [env.device-mix]`). **Content-team-owned** (data-model sample). Add any newly-cited fixtures to [docs/keyboard-index.md](../../docs/keyboard-index.md).
...
- [ ] T028 Run all of quickstart.md (Scenarios 1–6) end-to-end and record pass/fail evidence; reconcile any 036-harness stubs stood up in Phase 1 with the landed 036 work before merge.
