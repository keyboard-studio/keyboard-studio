...
- **IPA**: ~~A3=strong but the user prefers sequence modifiers~~ — closed in v1.1.1 by rule 3a (A3a=postfix → S-03 + S-04). See §7.1 A3a. (The §7.5 IPA row tests the *axis logic*; the actual `sil_ipa` keyboard is itself MML and falls under the Monolingual-scope note for new-authoring purposes.) Rule 3a is reachable in production via the Track 2 import path: `detectMarkInputOrderFromImport` (`packages/engine/src/strategy-selector/import-mark-order.ts`) scans an imported base's `KeyboardIR` for the postfix sequence-replace shape `any(base) + char > index(marked, N)` — the real `sil_ipa` rule shape, base typed first, mark-trigger key second — and records `markInputOrder="postfix"` via `axisFills` with `source: "import-derived"`, distinct from the script-class default-fill prior's `"script-class-prior"` tag (which must never emit `postfix`). This is base-derived, not script-derived, because postfix mark-input order isn't justifiable from script class alone. The generic survey-elicited path (a Phase B question) remains a separate, still-deferred half — see the phase-gated elicitation gaps note below.
