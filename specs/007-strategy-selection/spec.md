...
**Axis precedence — a recorded answer beats a fill or a prior derivation** *(amendment, [specs/052-marks-treatment-question](../052-marks-treatment-question/spec.md) FR-025)*. When more than one source can supply the same axis, the order is:

1. **An axis recorded by a survey station** — emitted on that phase's `SurveyPhaseResult.computedAxes` and merged into `session.axes` by `mergePhaseResults`. This is the author's own answer and it wins.
2. **A base-derived fill** (`axisFills` with `source: "import-derived"`, e.g. `detectMarkInputOrderFromImport`).
3. **The script-class default-fill prior** (`source: "script-class-prior"`).

`defaultFillAxes` structurally never overwrites an axis already present, so this precedence already *holds in behaviour*; it is stated here because behaviour that is only implicit is not a contract, and because spec 052's marks station is the first producer to depend on it. Concretely, the marks series now emits `computedAxes: { diacriticBehavior, markInputOrder }` derived from the recorded mark treatment (A4 from whether marks earn keys of their own and across how many mark families; A3a verbatim), so **A4 and A3a arriving from that station take precedence over any prior or fill**.

Precedence is not the whole story: a base keyboard's own mechanism is one of the marks station's proposal signals, so an author may knowingly answer against it — that is a legitimate override, not a conflict. What must never happen is the *recorded answer* and the *selected strategy* implying different mechanisms for the same mark (spec 052 FR-024/SC-011). That check therefore runs on the selection, **after** the tree fires, not on the raw axis vector — see `packages/engine/src/marks/strategy-reconcile.ts`. Note that deriving A4 correctly does not by itself prevent the contradiction: rule 5 below fires on `A3=strong AND A1 ∈ {medium, large}` alone, so a compose-as-you-type primary can still be selected when the recorded answer gives every marked character its own key.
...
**Revalidation record — 2026-07-29, [specs/052-marks-treatment-question](../052-marks-treatment-question/spec.md) FR-026 / SC-012.** The suite was re-run in full after spec 052 landed the single key-budget determination (`packages/contracts/src/keyBudget.ts`, with A7 redefined as a total, bijective projection of its three bands) and the marks-station A4/A3a projection. **Every one of the 13 covered keyboards selects exactly what it selected before — zero rows changed outcome, so no row required amendment.** Two things kept the table still:

- **The A7 projection is boundary-preserving.** `ralt-only → "RAlt only"` and `fully-booked → "fully booked"` are distinct, so the three rows at the intermediate band — `sil_euro_latin`, `armenian_mnemonic_r`, `russian_mnemonic_r` — stay at `"RAlt only"` and rule 10 (`A7 = "fully booked"` → `+S-08`) remains dormant for all three. Their expected primaries and secondaries are unchanged. (`armenian_mnemonic_r` and `russian_mnemonic_r` do carry `S-08`, from rule 8's per-rule secondaries, not from rule 10.)
- **A7 was redefined, not newly seeded.** Spec 052 deliberately does not seed `spareKeyAvailability` into the live `session.axes` (its research D2): A7 has no live producer today, so seeding it would be a first-time activation of gallery axis-ranking and rule 10 on real selections — a regression surface far outside a key-budget change. This table supplies A7 explicitly and is therefore untouched either way.

The A4/A3a projection likewise cannot move a row here: the table supplies its own full axis vector, so the new producer changes what reaches `selectStrategy` **in the live survey**, not what this fixture set feeds it.
...
- **IPA**: ~~A3=strong but the user prefers sequence modifiers~~ — closed in v1.1.1 by rule 3a (A3a=postfix → S-03 + S-04). See §7.1 A3a. (The §7.5 IPA row tests the *axis logic*; the actual `sil_ipa` keyboard is itself MML and falls under the Monolingual-scope note for new-authoring purposes.) Rule 3a has a base-derived supply path wired into the live studio pipeline: `detectMarkInputOrderFromImport` (`packages/engine/src/strategy-selector/import-mark-order.ts`) scans a base's `KeyboardIR` for the *unconditional* postfix sequence-replace shape `any(base) + char > index(marked, 1)` — base typed first, mark-trigger key second, the `index()` offset pointing at the leading `any()` — and records `markInputOrder="postfix"` via `axisFills` with `source: "import-derived"`, distinct from the script-class default-fill prior's `"script-class-prior"` tag (which must never emit `postfix`). This is base-derived, not script-derived, because postfix mark-input order isn't justifiable from script class alone. The studio consumes it at base instantiation (either track): `seedIrAxesFromBaseIr` (studio `workingCopyStore.ts`) seeds `markInputOrder="postfix"` onto `irAxes`, which flows through `defaultFillAxes` into `selectStrategy`, so rule 3a fires for such a base **in production**, not only in tests. **Reachability caveat:** the live `sil_ipa` postfix rules are all `if(option_key=…)`-guarded, and the codec currently classifies any `if(...)` context as opaque (`IF_OPTION_STORE`) — guarded rules become `RawKmnFragment`s and never reach `group.rules`, so the detector cannot see them; the guard-free variants in `sil_ipa` are all commented out. The detector therefore recognizes the postfix shape *in principle* and fires on any live guard-free postfix rule, but **no currently-shipping `sil_ipa` rule is reachable through it** until the codec's `if()`-guard handling is scoped separately. The generic survey-elicited path (a Phase B question) remains a separate, still-deferred half — see the phase-gated elicitation gaps note below.

The EuroLatin row remains in §7.5 as a regression fixture for the MML-derivation path, not as a v1 closure target.

**EuroLatin multi-family gap — restated as OPEN, reason unchanged (2026-07-29, [specs/052-marks-treatment-question](../052-marks-treatment-question/spec.md) FR-026 / US4 AC4).** Spec 052 requires each documented gap to be either closed with evidence or restated with its reason intact. This one is **restated, not closed**. Its reason is that EuroLatin is a massively multilingual keyboard and **MML-as-target is out of scope for v1**; matching SIL's S-02 choice would require modelling per-language sub-inventories the v1 axis vector does not carry. Nothing in spec 052 changes MML scope or adds per-language sub-inventory modelling, so closing the row would need evidence this feature does not produce. It stays in the table as the MML-derivation regression fixture it already is.

Note also what spec 052's A4 projection does *not* do here: it derives `"multi-family"` from the recorded treatment when marks across two or more mark families earn keys of their own, which is the same A4 value this row already supplies. Supplying that value from a new producer does not change which rule fires — rule 5 still precedes rule 6 on `A3=strong AND A1 ∈ {medium, large}` — so the row's outcome is unchanged.
...
- **Rule 3a / S-03 (A3a=postfix):** unlike the gaps above, the *base-derived* supply **is** wired — `detectMarkInputOrderFromImport` seeds A3a=postfix onto `irAxes` at base instantiation (`seedIrAxesFromBaseIr`, either track), so rule 3a fires in production for a base carrying unconditional postfix sequence-replace structure (this closes the reachability half of the caveat on the IPA row above). What remains deferred is the *generic survey-elicited* half: a `pb_mark_input_order` Phase B question already exists in the flow graph, but its answer is not yet threaded into `markInputOrder`. Wiring that probe into the axis vector — so a user without a postfix base can still declare A3a=postfix — is the outstanding follow-up (per §7.1's A3a row).
