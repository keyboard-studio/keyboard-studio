# @keyboard-studio/studio-poc

**Working POC of the in-browser compile + KMW preview pipeline.** Kept
on `main` as a dev-time reference and integration testbed, **not** as
the production UI. The spec'd authoring SPA lives in
[`@keyboard-studio/studio`](../studio/README.md); reuse components and
patterns from here as that package gets built up.

Run with `pnpm --filter @keyboard-studio/studio-poc dev`.
