// Top-level preview shell. Lifts the three pieces of UI state
// (baseKeyboard, modifiers, oskMode), wires the picker + toggles +
// iframe + metadata card together.
...
/**
 * Per-fixture try-this hints — keep concise; meant only to give a casual
 * user something to type so they can see the keyboard do something.
 */
const TRY_HINTS: Record<string, { intro: string; examples: string[] }> = {
  ...
  sil_devanagari_phonetic: {
    ...
    examples: ["a → अ", "k → क", "ka → क + virama? (try combos)", "i → इ"],
  },
};
