...
export function OSKFrame({
  ...
  // Forward text changes upward.
  useEffect(() => {
    if (onTextChange) onTextChange(textValue);
  }, [textValue, onTextChange]);

  // When a compile lands, push the .js URL to the iframe.
  useEffect(() => {
  ...
  // Forward desktop ↔ touch toggle.
  useEffect(() => {
    if (!engineReady) return;
    send({ type: "SET_OSK_MODE", mode: oskMode });
  }, [oskMode, engineReady, send]);

  // Out-of-scope script — render the stub instead of the iframe.
