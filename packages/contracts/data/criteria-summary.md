...
> **Canonical per-band recompute.** The numbers below are recomputed directly from `criteria.json`. The total and per-band split (148; 40/66/32/10) is **descriptive and expected to grow**; tests enforce schema-validity of every row and the four-band partition invariant (sum equals total, no orphans), **not the literal cardinality**. Prose mentions elsewhere (spec.md §11, CLAUDE.md, README) cross-link here; update this file when the data changes.
