...
export function parseTouchLayoutString(json: string): TouchLayoutIR {
  ...
  for (const platform of PLATFORM_ORDER) {
    ...
    for (const rawLayer of p.layer) {
      ...
      // One counter per LAYER, advanced in row-major order — the scope and the
      // order an address's occurrence is defined in.
      ...
      for (const rawRow of rawLayer.row ?? []) {
        ...
        for (const rawKey of rawRow.key ?? []) {
          ...
          keys.push(key);
          // Which key with this id, within this layer. Without it the index
          // silently collapsed: a layer with twenty-five `T_BLANK`s produced
          // twenty-five entries under ONE address, so twenty-four node
          // references were unreachable.
          ...
          nodeIds.push([
            touchKeyAddress(platform, id, key.id, occurrence),
            { kind: "touchKey", nodeId: key.nodeId },
          ]);
          if (key.sk) {
            for (const sk of key.sk) {
              nodeIds.push([
                touchSubKeyAddress(platform, id, key.id, "sk", sk.id, occurrence),
