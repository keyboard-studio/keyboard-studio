...
export interface PublishPROptions {
   ...
   * The fork of `keyboard-studio/keyboards` is created under this owner.
   */
  forkOwner: string;
  /**
...
export interface VerifyTokenResult {
  ...
  login?: string;
  /**
   * The account's display name, from the same `/user` response (spec 037 D7).
   *
   ...
   * name; never substitute `login`, because a handle is not a copyright holder.
   */
  name?: string;
  /**
   * The account's public email, from the same `/user` response (spec 037 D7).
   *
 ...
 *      `keyboard-studio/keyboards`, creates branch `add/<id>`, commits the
...
export interface OutputService {
  ...
  /**
   * Fork `keyboard-studio/keyboards`, push the virtual FS source tree to a new
