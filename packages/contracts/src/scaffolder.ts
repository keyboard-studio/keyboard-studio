...
export interface ScaffoldOptions {
  ...
  /**
   * Who to attribute the keyboard to (spec 064 US1).
   *
   ...
   * rights holder — the behaviour before spec 064 — is a false attribution, and
   ...
   * (spec 064 D2), and because tests must not read the clock — a time-dependent
   ...
   * `LICENSE.md` carries a notice this tool cannot read (spec 064 D5).
   *
...
export interface ScaffoldResult {
   ...
   * (spec 064 US2). Zero for a keyboard with no base license to retain.
   *
  ...
  /**
   * Set when the base HAS a copyright notice that could not be read (spec 064
