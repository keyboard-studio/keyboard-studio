...
import {
  definition,
  fixtures,
} from "../../../../src/survey/questions/b/pb_routing_branch.ts";
...
describe("pb_routing_branch — definition", () => {
  ...
  it("is engine-resolved (never rendered to user)", () => {
    expect((definition as Record<string, unknown>)["engine_resolved"]).toBe(
      true,
    );
  });
  it("routes non-roman to pb_non_roman_branch", () => {
    ...
    const nonRomanRoute = routes.find(
      (r) => r.condition === "ctx.routing_group == 'non-roman'",
    );
