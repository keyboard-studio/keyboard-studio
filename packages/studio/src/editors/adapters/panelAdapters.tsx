...
function contextFromIdentity(identity: IdentityLiteResult): SurveyContext {
  return {
    ...
    // spec 037 FR-016: publishing the contact here activates the Phase F
...
export function IdentityLiteAdapter({ onComplete }: EditorStepProps) {
  ...
  function handleComplete(result: SurveyPhaseResult, identity: IdentityLiteResult) {
    ...
    // spec 037 US1: the working copy is the single source the scaffolder reads
  ...
  return (
    <IdentityLite
      // spec 037 D7: pre-fill attribution from the authenticated profile so the
