...
describe("TouchGallery — propose (never silently route into) the by-key mode on entry (T073, FR-036)", () => {
  it("proposes the by-key mode ONLY when a key has no reachable output AND a character is unplaced — and does not switch modes until confirmed", async () => {
    seedKeyModeFixture({ inventory: ["ñ"], includeBrokenKey: true });
    await act(async () => {
      render(<TouchGallery onComplete={vi.fn()} onBack={vi.fn()} />);
    });

    // Proposed, not silently applied: the mode has NOT switched on its own.
    expect(screen.queryByTestId("touch-key-mode-back")).toBeNull();
    const acceptBtn = screen.getByTestId(
      "touch-key-mode-proposal-accept",
    );
    ...
    // Confirmed -> now it routes into key mode.
    expect(screen.getByTestId("touch-key-mode-back")).toBeTruthy();
    expect(useWorkingCopyStore.getState().touchEditorMode).toBe("key");
  });

  it("declining the proposal stays on the character walk and dismisses the banner", async () => {
    seedKeyModeFixture({ inventory: ["ñ"], includeBrokenKey: true });
    await act(async () => {
      render(<TouchGallery onComplete={vi.fn()} onBack={vi.fn()} />);
    });
    ...
    await act(async () => {
      fireEvent.click(
        screen.getByTestId("touch-key-mode-proposal-decline"),
      );
    });

    expect(useWorkingCopyStore.getState().touchEditorMode).toBe("character");
    expect(
      screen.queryByTestId("touch-key-mode-proposal-accept"),
    ).toBeNull();
  });

  it("control: broken key exists but every inventory character is already reachable elsewhere — no proposal (keys-with-no-output alone is not enough)", async () => {
    seedKeyModeFixture({
      inventory: ["a"],
      includeBrokenKey: true,
      coveringCharKey: "a",
    });
    await act(async () => {
      render(<TouchGallery onComplete={vi.fn()} onBack={vi.fn()} />);
  ...
  it("control: a character is unplaced but no key is broken — no proposal (unplaced characters alone is not enough); the character walk stays the default", async () => {
    seedKeyModeFixture({ inventory: ["ñ"], includeBrokenKey: false });
    await act(async () => {
      render(<TouchGallery onComplete={vi.fn()} onBack={vi.fn()} />);
    });
