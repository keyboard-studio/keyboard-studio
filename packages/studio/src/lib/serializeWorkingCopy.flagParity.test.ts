// spec-014 flag parity, flag-on-only half: touch re-propagation.
//
// The flag-on === flag-off emit parity for carve, add-gallery and touch inject
// runs in projectWorkingCopyVfs.flagParity.test.ts (projectWorkingCopyForOutput
// wraps that same projection and reads no flag of its own). This file holds the
// one surface that deliberately differs: re-propagation runs only with the flag on.
//
// Source of truth:
...
// full-spine proof.
//
// Carve + add-gallery emit byte-identically in both flag states (proved in
