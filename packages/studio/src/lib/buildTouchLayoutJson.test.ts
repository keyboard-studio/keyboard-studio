...
describe("buildTouchLayoutJson — Case A (router → IR path, undefined)", () => {
  ...
  // surviving phone platform must have exactly the scaffolder's canonical
  // layer shape (default/shift/numeric — mirrors scaffoldTouchLayout.test.ts's
  // "the phone platform has exactly 3 layers" assertion). This is the
  // strongest form of the R10 strip proof: it is not enough that "tablet" as
  // a platform id is absent — no trace of the shipped layout's content may
  // leak into the phone platform either.
  // -------------------------------------------------------------------------

  it("R10 (full assertion): a distinctive shipped platform+key never generated by the scaffolder is wholly absent, and the phone platform has exactly the canonical default/shift/numeric layer shape", () => {
    ...
    const parsed = JSON.parse(result.json!) as { phone: { layer: Array<{ id: string }> } };
    // Exactly the phone platform survives (mirrors scaffoldTouchLayout.test.ts's
    // canonical-shape assertion: "the phone platform has exactly 3 layers").
    expect(Object.keys(JSON.parse(result.json!) as Record<string, unknown>)).toEqual(["phone"]);
    expect(parsed.phone.layer.map((l) => l.id)).toEqual(["default", "shift", "numeric"]);
  });
});

// ---------------------------------------------------------------------------
// T020 — reseed replay reaches Case A: `mods.placements` must land on the
// projected phone default layer (proves `applyDesktopModifications` is
