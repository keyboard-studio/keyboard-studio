// FlowGraphView behaviour tests, via the real layout (no mocking):
//   - full render: every node renders inline, with no node cap or "Show more"
//     collapse (an earlier collapse clipped deep phases such as Phase B);
//   - the decision-trail path overlay (specs/053-decision-audit T040;
//     trail-ui.contract.md §5), below.
//
...
/** A linear chain of `n` nodes (q0..q{n-1}), for the full-render checks. */
function longChainGraph(n: number): FlowGraph {
  const nodes: GraphNode[] = Array.from({ length: n }, (_, i) =>
    node(`q${i}`, { flowId: "test_flow", isEntry: i === 0, isTerminal: i === n - 1 }),
  );
...
describe("FlowGraphView — full render (no collapse)", () => {
  it("a tall graph renders every node, with no Show more/less toggle and no height cap", () => {
    const { container } = render(<FlowGraphView graph={longChainGraph(8)} />);
  ...
  it("a short graph also renders fully with no toggle", () => {
    render(<FlowGraphView graph={longChainGraph(3)} />);
    expect(screen.queryByRole("button", { name: /show more|show less/i })).toBeNull();
    for (let i = 0; i < 3; i++) {
      expect(screen.getAllByText(`q${i}`).length).toBeGreaterThan(0);
    }
