...
/**
 * A {@link BaseBrowserService} extended with the spec 080 documentation
...
export interface BaseBrowserServiceWithDocProfile extends BaseBrowserService {
  /**
   * Classify `baseId`'s documentation completeness (spec 080 FR-008), from
...
/** The base's raw `.kps` text plus the folder it (and its source siblings) live under, kept for the doc-profile welcome probe (spec 080 R4) — the base's metadata fetch already paid for this text once. */
