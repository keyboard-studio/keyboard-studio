...
// spec 064 D7 / FR-002: `name` and `email` come from the SAME /user response the
...
describe("verifyToken — attribution fields (spec 064 D7)", () => {
...
describe("publishPR", () => {
  ...
  it("creates fork when fork does not exist (404)", async () => {
    ...
    routes.set(`POST ${API}/repos/keyboard-studio/keyboards/forks`, { ok: true, status: 202, body: { full_name: `${FORK_OWNER}/keyboards` } });
  ...
  it("opens the PR as a draft against keyboard-studio/keyboards:master", async () => {
    const capturedBodies: string[] = [];
    const base = buildMockFetch(happyPathRoutes());
    const captureFetch: GitHubFetchFn = async (url, init) => {
      if (init?.body !== undefined) capturedBodies.push(init.body);
