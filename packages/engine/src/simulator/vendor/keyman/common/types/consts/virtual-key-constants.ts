...
// Widened to match CLDRScanToVkey returning number | undefined.
export type KeyMapMaybeUndef = (number | undefined)[][];
 ...
 * @returns
 */
// Return type widened for strict-null compatibility.
export function CLDRScanToKeyMap(scans: KeyMap, badScans?: Set<number>): KeyMapMaybeUndef {
  return scans.map((row) => row.map((scan) => CLDRScanToVkey(scan, badScans)));
}

/** Convert one scan code to vkey, or undefined */
// Return type widened from `number` to `number | undefined` for strict-null compatibility.
export function CLDRScanToVkey(scan: number, badScans?: Set<number>): number | undefined {
