...
// textStore → syntheticTextStore → textStore causes SyntheticTextStore to extend an
// undefined TextStore under Node ESM module evaluation order.  In all call sites,
// `original` passed to buildTranscriptionFrom() is already a SyntheticTextStore snapshot
// (created by the caller via SyntheticTextStore.from()), so we pass it through directly
// to Transcription without re-snapshotting.  This preserves runtime correctness while
// eliminating the circular static import.  Upstream Keyman builds are CJS/bundled ESM
// where this cycle is harmless; see PROVENANCE.md §annotations.
...
export abstract class TextStore {
  ...
  buildTranscriptionFrom(original: TextStore, keyEvent: KeyEvent, readonly: boolean, alternates?: Alternate[]): Transcription {
    ...
    // VENDORED ANNOTATION: pass `original` directly instead of re-wrapping via
    // SyntheticTextStore.from() (see top-of-file comment re: ESM circular dep removal).
    // At all call sites, `original` is already a SyntheticTextStore snapshot; passing
    // it directly is semantically equivalent and avoids the static import of syntheticTextStore.ts.
    return new Transcription(keyEvent, transform, original as any, alternates);
  }
