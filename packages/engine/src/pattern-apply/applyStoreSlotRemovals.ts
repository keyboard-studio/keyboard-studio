...
/**
 * Full-IR analysis shared by {@link classifyStoreSlotEdit}, {@link describeStorePairing},
 * and {@link applyStoreSlotRemovals}. Exported (with {@link analyzeStores}) so a caller
 * that needs to classify or describe MANY stores against the same IR — e.g. the
 * studio's `toRailNodes` (once per store) or `recommendedRemovalChars` (once per
 * candidate store per candidate character) — can compute this once and pass it to
 * {@link classifyStoreSlotEdit} / {@link describeStorePairing}'s optional third
 * argument, instead of each call re-scanning every rule in the IR from scratch
 * (O(stores) or O(stores x chars) full-IR scans otherwise). Opaque to callers —
 * treat it as a token to pass back in, not a structure to read.
 */
 ...
 */
export function analyzeStores(ir: KeyboardIR): StoreAnalysis {
 ...
 * @param ir       The full IR (scanned for every rule's use of every store, unless `analysis` is supplied).
 * @param analysis Optional precomputed {@link analyzeStores} result — pass this when
 *                 classifying many stores against the same `ir` (see the doc comment on
 *                 {@link StoreAnalysis}) to skip re-scanning every rule per call.
 */
export function classifyStoreSlotEdit(
  store: IRStore,
  ir: KeyboardIR,
  analysis: StoreAnalysis = analyzeStores(ir),
): StoreSlotEditMode {
  return classifyStoreWithAnalysis(store, analysis);
}
 ...
 *
 * @param analysis Optional precomputed {@link analyzeStores} result — pass this when
 *                 describing many stores against the same `ir` (e.g. the studio's
 *                 `toRailNodes`, once per store) to skip re-scanning every rule per call.
 */
export function describeStorePairing(
  store: IRStore,
  ir: KeyboardIR,
  analysis: StoreAnalysis = analyzeStores(ir),
