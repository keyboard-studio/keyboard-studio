/**
 * Unit tests for the engine touchCoverage wrapper (spec 035, FR-008/SC-003).
 *
 * The traversal itself (producer mechanisms, layer reachability, star-labels,
 * U_ id decoding, spacers) is covered once, in
 * packages/contracts/src/touch-coverage.test.ts against computeTouchCoverage.
 * This file keeps a delegation smoke test, the NFD-stored layout case, and the
 * SC-001 real-corpus canary that threads the spec 063 options through the
 * wrapper.
 */
...
describe("touchCoverage", () => {
  it("delegates to computeTouchCoverage: covered chars drop out, each orphaned inventory occurrence is reported", () => {
  ...
  it("covers an NFC inventory char from an NFD-stored layout string", () => {
    // "e" + combining acute accent (U+0065 U+0301), NFD form of "é". Not a
    // duplicate: the same-titled contracts case stores the precomposed form.
    const nfdText = "é";
    const layout = makeLayout([
