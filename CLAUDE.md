...
**kbgen (placement seeder) — ESM TypeScript, lives in `utilities/kbgen/`.** A standalone Node CLI that derives data-driven character placement (which key, which mechanism) from pinned Unicode/CLDR signals and emits a `placement-map.json`. Ported from CommonJS plain-JS to ESM TypeScript (issue #132): `tsconfig.json` extends `tsconfig.base.json`; build/typecheck/test scripts wired (`pnpm --dir utilities/kbgen build` / `typecheck` / `test`); `test/anchors.test.ts` runs under vitest. Intended to become an engine deliverable (a seeder ahead of the survey, §8 Phase B), but does not yet conform to `packages/contracts` types and implements only S-01/S-08 of the §7.3 catalog. **Kept out of `pnpm-workspace.yaml`** (and out of `packages/*`) until the `PlacementMap` contract type lands (issue #133, blocked by #131) — adding it to the workspace before that lands would make `pnpm -r` traverse a non-conformant package. Build/typecheck/test via `pnpm --dir utilities/kbgen ...`. Conformance path + decisions: [utilities/kbgen/INTEGRATION.md](utilities/kbgen/INTEGRATION.md). Do not treat it as a built workspace package.
...
- **Standalone utilities.** `utilities/*` (kbgen, supportability-scanner, smoke-artifact, spec-trace, km-triage-app, Template Cleanup) are deliberately kept out of `packages/*` so they don't trip `pnpm -r`. `kbgen` is now ESM TypeScript with its own `tsconfig.json` — run `pnpm --dir utilities/kbgen build/typecheck/test` directly, or use `tsx` for dev. Other utilities may still use `tsx` (see each tool's tsconfig). Do not treat any of them as built workspace packages.
