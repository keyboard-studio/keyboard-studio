...
**E2E:** Playwright specs live under `packages/studio/e2e/`, run against the global Playwright CLI (`cd packages/studio && npx playwright test <spec>`; all specs import from `"playwright/test"`, not `@playwright/test` — the scoped package does not resolve to the global CLI in this environment). **Status (2026-07-15): every walk-from-scratch spec is currently blocked by a stale survey prelude** — the copied `driveIdentityLite` helpers still target the old identity-lite first field (`#il_language_autonym`), but the app now opens with the 036 glottolog language-identify flow, so [`carve.spec.ts`](packages/studio/e2e/carve.spec.ts) and [`copy-edit.spec.ts`](packages/studio/e2e/copy-edit.spec.ts) (previously live/passing) fail at that locator, and [`touch-derivation-us1.spec.ts`](packages/studio/e2e/touch-derivation-us1.spec.ts) (spec 035 US1: bambara import-and-adapt walk) and [`touch-derivation-us2.spec.ts`](packages/studio/e2e/touch-derivation-us2.spec.ts) (spec 035 US2: pid_piaroa reseed walk + the explicit-reseed AS4 variant) are `.skip`-ped with their unblock recipes at the top. The shared fix is to extract the prelude into `e2e/helpers/surveyFlow.ts` and update it once for the new flow. `import-improve.spec.ts` (Track 2) remains `.skip`-ped for its own reasons (recipe at the top of the file). Studio code exposes a flag-gated `window.__ksE2E__` test hook ([`packages/studio/src/lib/e2eHook.ts`](packages/studio/src/lib/e2eHook.ts)), active only under `VITE_E2E=1` or `?e2e=1`.
...
- **Standalone utilities.** `utilities/*` (kbgen, supportability-scanner, smoke-artifact, spec-trace, km-triage-app, hermes, Template Cleanup, `facet-index` + `facet-index-lint`) are deliberately kept out of `packages/*` so they don't trip `pnpm -r`; run them with `tsx` (see each tool's tsconfig). Do not treat them as built workspace packages. `facet-index` ([spec 036](specs/036-keyboard-facet-index/)) scans the sibling `keymanapp/keyboards` corpus and emits the offline, deterministic `docs/keyboard-facet-index.json` (+ `.md` companion) — a per-keyboard facet index (036 landed the `script` facet; [spec 037](specs/037-facet-classifiers/) added the `strategy-fingerprint` and `target-mix` classifiers). Classifiers live flat under `utilities/facet-index/*-classifier.ts`, registered as `{ classify, fallback }` pairs in `DEFAULT_CLASSIFIERS` keyed by facet id. A facet YAML can land ahead of its classifier (e.g. spec 039's construction facets); the default build fails loud on such a def, and `--classified-only` builds the artifact scoped to facets that have a classifier (how the shipped index is built today). `facet-index-lint` is its plain-node artifact validator, wired into `pnpm lint`.
