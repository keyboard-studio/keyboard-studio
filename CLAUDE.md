...
- **Criteria data + count:** `packages/contracts/data/criteria.json` is the data; the per-band recompute lives in `criteria-summary.md`; the current count (149; 40/67/32/10) is **descriptive and expected to grow**. Tests enforce schema-validity of every row (via `CriterionSchema`) and the four-band partition invariant (sum of band counts === total, no orphans) — **not the literal cardinality**. Prose mentions (spec §11, this file, README) cross-link rather than re-derive.
