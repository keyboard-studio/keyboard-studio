...
**E2E:** Playwright specs live under `packages/studio/e2e/`. Two are live — [`carve.spec.ts`](packages/studio/e2e/carve.spec.ts) (imports `bj_cree_woods`, carves an opaque rule, asserts the IR and emitted `.kmn`) and [`copy-edit.spec.ts`](packages/studio/e2e/copy-edit.spec.ts) (walks Track 1 identity-lite -> base picker -> Phase B -> emit, asserts the downloaded `.zip` contents) — run them against the global Playwright CLI: `cd packages/studio && npx playwright test carve.spec.ts copy-edit.spec.ts`. Both import from `"playwright/test"`, not `@playwright/test` (the scoped package does not resolve to the global CLI in this environment). The remaining spec (`import-improve.spec.ts`, Track 2) is still `.skip`-ped (it carries its unblock recipe at the top: Playwright wiring is done, but Track 2 import-UI liveness and the re-import round-trip path still need confirming before removing the inner `.skip` marks). Studio code exposes a flag-gated `window.__ksE2E__` test hook ([`packages/studio/src/lib/e2eHook.ts`](packages/studio/src/lib/e2eHook.ts)), active only under `VITE_E2E=1` or `?e2e=1`.
