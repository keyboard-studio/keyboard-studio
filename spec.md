...
**Workflow ordering (hybrid).** The phases below are reached in a **hybrid** order: base resolution comes first so the studio can suggest a base keyboard immediately, after which track choice splits the two authoring paths; the engaging character/gallery work then precedes the deferred paperwork. The adopted sequence is:

- **Base resolution** — the studio presents a language + script search prompt and suggests a base from [docs/keyboard-index.md](docs/keyboard-index.md) keyed on the *(language, script) pair* (so a romanization suggests a Latin base, not the language's default-script base), else lets the user pick one, else starts from the bundled US-QWERTY fallback (the "blank" fallback is the bundled `basic_kbdus` layout — the studio starts from *its* IR, not an empty `.kmn`; an empty file would rely on the OS baseline, not emit QWERTY itself). Then **parse to IR**, **scaffold over IR**, and the **carve gallery** (steps 2–4 below).
- **Track choice** — the author selects Track 1 (new keyboard from base) or Track 2 (adapt existing keyboard). Track 2 proceeds directly to base-derived prefill; Track 1 continues with identity-lite and project-name below.
- **Identity-lite (Track 1 only)** — language autonym, English name, **ISO 639 code**, and **target script** (a short subset of Phase A). **Language and script are decoupled:** the keyboard's target is a *(language, script) pair*, and script is its own question — not derived from the language's default. Alongside the language's default script(s), identity-lite offers **romanization (Latin)** and **IPA** (`-fonipa`) and "another script" as first-class options, so an alternate-script keyboard (e.g. a `hi-Latn` romanization, or an IPA keyboard) is a normal path. The *chosen* script — carried in the BCP47 tag (`-Latn`, `-fonipa`) — drives routing (§9), A2 (§7.1), base suggestion, and the inventory diff; the language does not. Track 2 skips this step because identity is already known from the existing keyboard.
- **Project-name (Track 1 only)** — display name and auto-derived keyboardId for the new keyboard. Track 2 skips this step; the existing keyboard's ID and display name are preserved.
- **Base-derived prefill** — routing group, script class (A2), spare-key availability (A7), and BCP47 are pre-filled from the base as confirmations (Sec 5 "Base-derived pre-fill"), not re-asked. Both tracks converge here.
...
1. **Source selection.** The source-selection browser offers the user one of four sources for the session: the bundled US-English fallback (preselected), any `release/basic/*` layout, any other `keymanapp/keyboards/release/` keyboard, or an uploaded `.kmn`. The user picks exactly one. There is no multi-source merge. In the hybrid order this step is the **first step** (base resolution): when a base already covers the entered *(language, script) pair* it is **suggested** here from [docs/keyboard-index.md](docs/keyboard-index.md); the user may accept the suggestion, pick another, or fall through to the US-QWERTY default. Track choice (Track 1 vs. Track 2) immediately follows base confirmation.
...
**Hybrid ordering note (Track 1).** In the hybrid flow, identity-lite (autonym, English name, ISO 639 code, target script) and project-name (display name + auto-derived keyboardId) are collected after base resolution and track choice — they are not the entry point. The **script subtag is resolved at identity-lite, not deferred further** — §9 routing and the A2 script class depend on it, so identity-lite's script answer (refined by the base's BCP47 on selection) fixes the BCP47 script subtag before the gallery phases run. Only the **copyright holder, region, and the optional provenance metadata** are deferred to the documentation stage alongside Phase F (see the Workflow ordering preamble), so the engaging character/gallery work is not gated behind paperwork. The scaffolder (step 3) runs before that stage: it propagates `header.bcp47` from the resolved tag and seeds `header.keyboardId` / display name from the project-name step as a **provisional** value, which the documentation stage finalizes before output (step 15 is gated on documentation-stage completion, so a PR never carries an empty copyright or BCP47). The routing detection in this step is pre-filled from the chosen base as a confirmation (Sec 5 "Base-derived pre-fill"), not asked blank. **Track 2** reaches Phase A for routing confirmation only — identity fields are pre-populated from the existing keyboard's IR and are confirmations, not blank prompts.
