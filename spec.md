...
- **Signed-in** — local store *plus* an account-keyed backing store for cross-device resume (§16). Signing in migrates the guest's local projects to the account; the local store remains as an offline cache, so authoring continues to work without network. Signing out stops syncing but does not delete local projects.
...
- **Hosting and deployment** — infrastructure is left to the operator; this project ships a static SPA. **(v1.4.0 exception, PROPOSED):** an optional, minimal backing service is permitted solely for the two responsibilities below, and nothing else:
  ...
  2. Account-keyed save & resume (§12) — email-identity login plus a per-account snapshot store keyed by `projectId`.
