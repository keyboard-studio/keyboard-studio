...
- **Signed-in** — local store *plus* an account-keyed backing store for cross-device resume (§16). Two identity providers are supported, with **GitHub SSO the preferred, one-click default** and **email + password the fallback**: **GitHub SSO** (reuses the OAuth backend the PR delivery path already needs, with login-only scopes; the PR-delivery scope is requested incrementally only when the author attempts a PR delivery) is offered first, and **email + password** keeps the studio fully usable without a GitHub account. GitHub is preferred but never required. Signing in migrates the guest's local projects to the account; the local store remains as an offline cache, so authoring continues to work without network. Signing out stops syncing but does not delete local projects.
...
- **Hosting and deployment** — infrastructure is left to the operator; this project ships a static SPA. **(v1.4.0 exception, PROPOSED):** an optional, minimal backing service is permitted solely for the two responsibilities below, and nothing else:
  ...
  2. Account-keyed save & resume (§12) — identity (**GitHub SSO preferred, email+password fallback**) plus a per-account snapshot store keyed by `projectId`.
  ...
  **Data protection (GDPR).** As the only component that holds personal data, the backing service must be designed so these are possible and the studio must not preclude them: account creation is opt-in and consent-based; only the minimum personal data needed for resume is stored (an identity handle — GitHub login or email — plus the user's own snapshots), per data-minimization; signed-in users can **export and erase** their account data and snapshots on request (access / portability / erasure); snapshots carry a defined retention limit (storage limitation); only strictly-necessary cookies are used (the httpOnly session cookie — no tracking or analytics cookies); and third-party processors (host, datastore) are bound by a data-processing agreement. The legal instruments (privacy notice, DPA, lawful-basis record) are the operator's responsibility; guest / browser-local remains the privacy-by-default baseline, holding no server-side personal data.
