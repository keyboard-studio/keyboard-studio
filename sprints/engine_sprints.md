...
**#232** `feat(contracts): KeyboardIR schema + IR node types + ImportStatus / ImportReport (#5b)` — *unassigned*
...
**#248** `feat(engine): identity-stub mutation applyIdentityStubMutation()` — *unassigned*
...
**#249** `feat(studio): Phase A first-3-question identity renderer (subset of #48)` — *unassigned*
...
**#192** `bug(flows): Phase A script_family divergence (flow 5 vs ScriptFamily type 7) + missing §16 stub gate` — *unassigned*
...
**#233** `feat(engine): KeyboardIR codec — .kmn/.kvks/.keyman-touch-layout parser and emitter` — *unassigned*
...
**`feat(engine): replace identity stub with IR-layer mutation`** — *new issue from KS-S1 close* — *unassigned*
...
**#85** `chore(flows): SurveyAnswer.value is opaque string — boolean/select encoding unclear` — *unassigned*
...
**#234** `feat(engine): Pattern recognizer — lift IR node clusters into Patterns with origin='recognized'` — *unassigned*
...
**#235** `feat(studio): Carve gallery — keep/edit/delete card view over the working IR` — *unassigned*
...
**#236** `feat(engine): Layer A' import-fidelity checks I1-I5 in @keymanapp/kmn-validator` — *unassigned*
I1 parse completeness, I2 round-trip, I3 header preservation, I4 opaque count budget, I5 sidecar hash. Pairs with #233 codec.

**#239** `feat(output): .kmn.imported sidecar — include in .zip and OAuth working tree, exclude from PR commit` — *unassigned*
...
**#106** `chore(scaffolder): scaffold() promises 'Layer-C-clean' but returns VirtualFS only` — *unassigned*
...
**#183** `feat(compiler): Deterministic simulate() API for tests, fixtures, and docs` — *unassigned*
Successor to the closed #18. Powers reproducible preview output for test fixtures and the §7.5 self-check table.

**#156** `feat(engine): Section-18 DISCUS design-heuristic lint checks + TouchLayout type` — *unassigned*
...
**#237** `feat(tools): Supportability scanner CLI — codec + Layer A' over release/` — *unassigned*
...
**#226** `feat(flows): Phase C RTL direction-marks key placement` — *unassigned*
...
**#53, #54** `Day-7 E2E smoke test — real Keyman Developer build` — *unassigned*
After #32 passes.
...
**#132** `chore(tools): Port kbgen to ESM TypeScript and wire into workspace` — *unassigned*
**#133** `feat(contracts): Add placement-map type from joint session` — *unassigned*
...
**#141** `feat(engine): CharacterDiscoveryService — text harvest + CLDR-exemplar picker` — *unassigned*
