...
**#240** `feat(content): Pattern recognizer rule curation — first-pass S-01..S-09` — *done*
...
**#212** `docs(keyboards): Add pattern-YAML links to scan_report.md` — *done*
...
**#240** `feat(content): Pattern recognizer rule curation — S-03..S-09 completion` — *done*
...
**#241** `chore(criteria): Add criteria.md rows for import-attribution PR body and sidecar exclusion` — *done*
...
**#204** `bug(patterns): indic-pre-base-vowel test-vector contract violation + appliesTo overreach` — *done*
...
**#41** `feat(patterns): Reorder pattern: indic-pre-base-vowel` — *done*
...
**#191** `feat(flows): Phase B character-inventory coverage gaps` — *done*
...
**#52** `feat(flows): Phase F help docs (welcome, tips, credits)` — *done*
Phase F survey YAML: prose-collection questions for welcome HTML, usage tips, credits, license note.

**#55** `feat(patterns): Validation safety net — kmc CLI green-light for every filled_kmn` — *done*
...
**#70** `chore(criteria): populate optional automation-hook fields (lintRuleId, scaffolderRule, etc.)` — *done*
...
**#120** `chore(criteria): 13 criteria flagged for re-review — file individual decisions` — *done*
