...
| [`packages/glottolog/`](packages/glottolog/) | Offline, pinned Glottolog language-classification catalog + the keyboard-base bridge — when a language has no keyboard, ranks candidate bases from close genealogical relatives (same-script) ahead of a plain script fallback. Contracts-only; no engine/studio import. |
